...
const columns: ColumnDef<FAPeerRow, unknown>[] = [
  ...
  { accessorKey: 'roe', header: zHeader('ROE (z)', 'ROE (Return on Equity): net profit as a % of shareholder equity.'), cell: (i) => i.getValue<number | null>()?.toFixed(2) ?? '—' },
  { accessorKey: 'roce', header: zHeader('ROCE (z)', 'ROCE (Return on Capital Employed): operating profit as a % of total capital employed (equity + debt).'), cell: (i) => i.getValue<number | null>()?.toFixed(2) ?? '—' },
  { accessorKey: 'debt_to_equity', header: zHeader('D/E (z)', 'D/E (Debt-to-Equity): total debt divided by shareholder equity — a leverage measure.'), cell: (i) => i.getValue<number | null>()?.toFixed(2) ?? '—' },
  { accessorKey: 'pe_ratio', header: zHeader('PE (z)', 'PE (Price-to-Earnings): share price divided by earnings per share.'), cell: (i) => i.getValue<number | null>()?.toFixed(2) ?? '—' },
]
