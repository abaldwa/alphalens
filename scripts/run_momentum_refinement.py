"""
scripts/run_momentum_refinement.py

Phase: FeatureBacklog.md ML38 — momentum strategy refinement
Owner: Platform / Backtest
Consumers: operator CLI (`python3 -m scripts.run_momentum_refinement`)

2026-07-14 user request: having found Band3 (rank 100-150) / 6mo lookback
/ monthly rebalance / 15 stocks as the highest-CAGR variant in the
original 240-variant grid, drill into the neighborhood around it to see
if CAGR can be pushed further. Fixes the rank band at 100-150 and sweeps:
  - top_n: 11..18 stocks
  - lookback: 4,5,6,7,8 months
  - grace: how long a stock that drops out of the top-N is held before
    being force-sold, expressed in rebalance cycles (the engine's native
    unit — see MomentumBacktester's grace_cycles). The user asked for
    "sell immediately / after 1 month / after 2 months"; since a
    "month" isn't a fixed number of rebalance cycles once rebalance
    frequency itself is one of the swept dimensions, this is mapped onto
    the engine's own unit as 0 / 1 / 2 rebalance cycles (immediate /
    held one more rebalance / held two more rebalances) rather than
    forcing a calendar-day reinterpretation the engine doesn't natively
    support.
  - rebalance: 30, 45, 60, 75 calendar days, converted to trading days
    at the codebase's existing 21-trading-days-per-month convention
    (i.e. trading_days = round(calendar_days * 21 / 30)).

8 x 5 x 3 x 4 = 480 variants, single run each (no SIP pass — out of
scope for this CAGR-focused sweep; see the original 240-variant grid in
scripts/run_momentum_experimentation.py for the SIP comparison).
"""
...
TOP_N_OPTIONS = [11, 12, 13, 14, 15, 16, 17, 18]
LOOKBACK_MONTHS_OPTIONS = [4, 5, 6, 7, 8]

# [H4, 2026-08-18] grace_cycles no longer exists on MomentumAdapter --
# deprecated by §19. This axis is collapsed to one placeholder value
# (top_n/lookback/rebalance stay real, independent axes) rather than
...
REBALANCE_CALENDAR_DAYS = [30, 45, 60, 75]
...
def main():
    ...
    out_path = REPORTS_DIR / f"momentum_refinement_{now_ist().strftime('%Y%m%d_%H%M%S')}.json"
