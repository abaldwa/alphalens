...
def run_dynamic_report(years_back: int = 10) -> Dict:
            ...
            for lookback_months in LOOKBACK_MONTHS:
                ...
                for rebalance_name, rebalance_days in REBALANCE_PERIODS.items():
                    for top_n in TOP_N_OPTIONS:
                        logger.info(
                            "category=%s band=%d lookback=%dmo rebalance=%s top_n=%d",
                            strategy_name, band_id, lookback_months, rebalance_name, top_n,
                        )
                        result = _run_variant(
                            kwargs, price_panel, yearly_universes, lookback_days, rebalance_days,
                            starting_capital=STARTING_CAPITAL, investable_pct=INVESTABLE_PCT, top_n=top_n,
                        )
                        sip_result = _run_variant(
                            kwargs, price_panel, yearly_universes, lookback_days, rebalance_days,
                            starting_capital=SIP_STARTING_CAPITAL, investable_pct=INVESTABLE_PCT, top_n=top_n,
...
def main():
    ...
    report = run_dynamic_report(years_back=args.years_back)
