...
# PHASE 0 — Infrastructure & Data Foundation (Weeks 1–4)
...
📋 **PROMPT:**
```
Read docs/CLAUDE.md, docs/12_platform_architecture.md, and docs/specs/08_specifications.md sections SPEC-SYS-001 through SPEC-DS-007.
...
5. requirements/phase0.txt: pandas, numpy, pyarrow, duckdb, sqlalchemy, requests, beautifulsoup4, APScheduler, python-dotenv, pytest, pytest-cov
...
Every file must have a module-level docstring referencing its SPEC-ID per SPEC-TRACE-002. No hardcoded values anywhere — all constants in config/settings.py.
```

✅ **TEST:**
```bash
python -c "from config.settings import *; print('Settings OK')"
python -c "from config.universe import load_universe; print(f'Universe loader OK')"
...
📋 **PROMPT:**
```
Read docs/12_platform_architecture.md (Six Stores section) and docs/specs/08_specifications.md sections SPEC-DS-001 through SPEC-DS-007, SPEC-PIPE-003.
...
# path. `python datastore/schema/create_normalised.py` puts only
...
python -m datastore.schema.create_signals
python -c "import duckdb; conn = duckdb.connect('datastore/normalised/alphalens.duckdb'); print(conn.execute('SHOW TABLES').fetchall())"
...
📋 **PROMPT:**
```
Read docs/13_scheduler_resilience.md and docs/specs/08_specifications.md sections SPEC-SCHED-001 through SPEC-SCHED-011.
...
📋 **PROMPT:**
```
Read docs/specs/08_specifications.md SPEC-PIPE-001, SPEC-PIPE-005, SPEC-PIPE-006 and docs/specs/API_SPEC.md.
...
# Manual spot check:
python -c "from ingestion.scrapers.bhavcopy import download_bhavcopy; df = download_bhavcopy('2025-01-15'); print(df.shape, df.columns.tolist())"
```
...
📋 **PROMPT:**
```
Read docs/03_data_pipeline.md sections on historical backfill and docs/specs/08_specifications.md SPEC-PIPE-001, SPEC-PIPE-002.
...
# "Post-handoff bug #3"):
python -m ingestion.scrapers.fyers_backfill login
...
python -m ingestion.scrapers.fyers_backfill exchange "<paste the redirected URL here>"
...
```bash
python -m ingestion.backfill_runner --from 2020-01-01 --to 2025-12-31
# -- NOT `python ingestion/backfill_runner.py ...` (direct script path):
...
# Verify:
python -c "import duckdb; conn = duckdb.connect('datastore/normalised/alphalens.duckdb'); print(conn.execute('SELECT COUNT(*) FROM ohlcv_adjusted').fetchone())"
...
*execution environment* for the existing step functions, not a separate codepath.
...
python ingestion/quality/baseline_runner.py  # run after backfill
```
...
📋 **PROMPT:**
```
Run the Phase 0 gate check. Read docs/14_engineering_standards.md section "Phase Gate Checklists" for Phase 0→1 criteria.
...
7. Check all Python files have module-level docstrings with SPEC-ID references
...
📋 **PROMPT:**
```
Read docs/01_features.md Phase 1 features section and docs/specs/08_specifications.md SPEC-FEAT-001 through SPEC-FEAT-005.

Build the core feature computation modules. All computation must be fully vectorized — no Python loops over individual stocks:

1. features/technical.py — 76 core technical features:
...
2. features/calendar.py — 7 calendar features:
...
3. features/macro_features.py — 14 macro features:
...
5. tests/unit/test_features_technical.py:
...
✅ **TEST:**
```bash
pytest tests/unit/test_features_technical.py -v
python -c "
...
print(df.isnull().sum().sum(), 'nulls (should be low)')
"
```
...
📋 **PROMPT:**
```
Read docs/02_models.md section on M-01 HMM and docs/specs/08_specifications.md SPEC-MODEL-001.

Build the HMM regime detector:
1. systems/ml_signal_engine/models/hmm/regime_detector.py — HMMRegimeDetector class:
...
3. tests/integration/test_hmm_pipeline.py:
...
All docstrings reference SPEC-MODEL-001, SPEC-SOLID-003.
```

✅ **TEST:**
```bash
pytest tests/unit/test_hmm.py tests/integration/test_hmm_pipeline.py -v
```
...
📋 **PROMPT:**
```
Read docs/01_features.md P&D features section and docs/specs/08_specifications.md SPEC-MODEL-006, SPEC-FEAT-004.
...
📋 **PROMPT:**
```
Read docs/specs/08_specifications.md SPEC-MODEL-002, SPEC-MODEL-003, SPEC-BT-001 through SPEC-BT-004 and docs/04_backtesting.md.
...
📋 **PROMPT:**
```
Read docs/02_models.md sections M-02, M-03, M-04, M-05 and docs/specs/08_specifications.md SPEC-MODEL-003, SPEC-MODEL-004, SPEC-MODEL-007.
...
2. systems/ml_signal_engine/models/signal/signal_5d.py — Signal5DModel:
   ...
   - Input: 98 Phase 1 features (76 technical + 14 macro + 7 calendar + 1 HMM regime)
...
python systems/ml_signal_engine/inference/train_all_phase1.py --folds 2 --quick  # 2 folds for quick test
```
...
📋 **PROMPT:**
```
Read docs/02_models.md M-07 section and docs/specs/08_specifications.md SPEC-MODEL-002.
...
python backtest/run_phase1_backtest.py --quick --folds 2
...
📋 **PROMPT:**
```
Read docs/12_platform_architecture.md API Groups section and docs/specs/08_specifications.md SPEC-DS-002, SPEC-UI-001 through SPEC-UI-005.
...
curl "http://localhost:8000/api/v1/signals/ml/top_buys/2025-01-15" | python -m json.tool
python dashboard/screens/daily_dashboard.py
```
...
📋 **PROMPT:**
```
Run the Phase 1 gate check. Read docs/14_engineering_standards.md Phase 1→2 gate criteria.

Check and report PASS/FAIL:
1. Run pytest tests/ --cov=. — must show >= 80% coverage
2. Run backtest integrity check: python backtest/run_phase1_backtest.py --check-only
3. Verify daily pipeline timing: python ingestion/scheduler/daily_pipeline.py --dry-run --timing — must complete simulation in < 90 minutes
4. Verify P&D hard block: python -c "from systems.ml_signal_engine.models.pnd.pnd_detector import PnDDetector; print('PND block at:', PnDDetector.BLOCK_THRESHOLD)"
...
📋 **PROMPT:**
```
Read docs/03_data_pipeline.md fundamentals section, docs/specs/08_specifications.md SPEC-PIPE-003 (PIT — CRITICAL), SPEC-FEAT-002.
...
📋 **PROMPT:**
```
Read docs/01_features.md MF holdings and corporate action features sections and docs/specs/08_specifications.md SPEC-FEAT-004.
...
python -c "from features.mf_holdings import compute_mf_features; print('MF features OK')"
```
...
📋 **PROMPT:**
```
Read docs/01_features.md F&O features section and docs/specs/08_specifications.md SPEC-FEAT-004.
...
"
python systems/ml_signal_engine/inference/retrain_phase2.py --quick --folds 2
```
...
📋 **PROMPT:**
```
Read docs/02_models.md M-08 section, docs/01_features.md multibagger features, and docs/specs/08_specifications.md SPEC-MODEL-001.
...
📋 **PROMPT:**
```
Read docs/Forensic_Accounting_ML_Specification.md (full document) and docs/specs/08_specifications.md SPEC-MODEL-009, SPEC-MODEL-010.
...
📋 **PROMPT:**
```
Read docs/Fundamental_Data_Sourcing_Guide.md Trendlyne and Tijori sections. Read docs/12_platform_architecture.md Data Flow Matrix.
...
python backtest/run_phase2_backtest.py --quick --folds 2
curl "http://localhost:8000/api/v1/watchlist/current" | python -m json.tool
```
...
📋 **PROMPT:**
```
Run the Phase 2 gate check. Read docs/14_engineering_standards.md Phase 2→3 gate criteria.

Check and report PASS/FAIL:
1. pytest tests/ --cov=. — coverage must be >= 80%
2. python backtest/run_phase2_backtest.py --report — Mean Sharpe must be > 1.0 across all folds
3. python -c "from systems.ml_signal_engine.models.forensic.classical_scores import *; print('Forensic OK')" — must work
...
6. Check Trendlyne API key set: python -c "import os; assert os.getenv('TRENDLYNE_API_KEY'), 'Missing'"
...
📋 **PROMPT:**
```
Read docs/01_features.md Phase 3 features and docs/specs/08_specifications.md SPEC-FEAT-001 through SPEC-FEAT-005.
...
5. Update features/matrix_builder.py to Phase 3 (330 features total):
   - Add all new features; document in docs/01_features.md feature catalog update
...
📋 **PROMPT:**
```
Read docs/02_models.md sections M-11, M-12 and docs/05_ml_algorithms.md deep learning section.
...
python -m systems.ml_signal_engine.models.deep.tft_model --quick --epochs 2
```

⚠️ **MANUAL — Schedule full overnight TFT training:**
```bash
nohup python systems/ml_signal_engine/inference/train_deep_models.py \
...
📋 **PROMPT:**
```
Read docs/02_models.md M-13, M-14 sections and docs/04_backtesting.md Phase 3 section.
...
2. systems/ml_signal_engine/models/training/feature_selection.py — M-14 TabNet validator:
   ...
   - List pruned features in docs/01_features.md as "Phase 3 pruned — reason"
...
python backtest/run_phase3_backtest.py --quick --folds 2
...
📋 **PROMPT:**
```
Read docs/15_future_applications.md APP-1 section (SPEC-TA-001 through SPEC-TA-008) and docs/12_platform_architecture.md Consumer Systems section.
...
4. systems/technical_analysis/screener/ta_screener.py:
   ...
   - Pre-built templates from docs/15_future_applications.md SPEC-TA-005:
...
📋 **PROMPT:**
```
Read docs/15_future_applications.md APP-3 section (SPEC-VAL-001 through SPEC-VAL-010) and docs/Damodaran_Valuation_Module.md (full document).
...
4. systems/damodaran_valuation/dcf/wacc_calculator.py:
   ...
   - Synthetic credit rating for unrated companies using ICR table from docs
...
# Test one valuation:
curl "http://localhost:8004/api/v1/valuation/RELIANCE?as_of=2025-01-15" | python -m json.tool
```
...
2. python backtest/run_phase3_backtest.py --report — Sharpe improvement vs Phase 2 must be >= 0.1
...
6. Verify DataStore API documents all endpoints: curl http://localhost:8000/docs (should load Swagger UI)
...
📋 **PROMPT:**
```
Read docs/15_future_applications.md APP-2 section (SPEC-FA-001 through SPEC-FA-008) and docs/Fundamental_Data_Sourcing_Guide.md.
...
📋 **PROMPT:**
```
Read docs/15_future_applications.md APP-4 section (SPEC-FOREN-001 through SPEC-FOREN-012) and docs/Forensic_Accounting_ML_Specification.md.
...
📋 **PROMPT:**
```
Check RL prerequisites. Read docs/02_models.md M-15 section and docs/11_phase_delivery_plan.md Phase 4 prerequisites.
...
```
Read docs/02_models.md M-15 PPO RL section and docs/specs/08_specifications.md SPEC-MODEL-001.
...
📋 **PROMPT:**
```
Read docs/02_models.md M-15 bootstrapping stages. Build Stages 2 and 3 only.
...
This is CPU-intensive training. Schedule overnight:
  nohup python systems/ml_signal_engine/models/rl/ppo_trainer.py --stages 2,3 > logs/rl_training.log &
```
...
📋 **PROMPT:**
```
Read docs/02_models.md M-16 section and docs/specs/08_specifications.md SPEC-OBS-001 through SPEC-OBS-005.
...
5. Final documentation update:
   - Update docs/CLAUDE.md with Phase 4 status
   - Update docs/07_truthful_expectations.md with actual vs expected performance (honest)
   - Generate docs/SYSTEM_STATUS.md: all 16 models, 4 consumer systems, current status
```
...
4. Run full backtest: python backtest/run_phase3_backtest.py --report — Sharpe > 1.0
5. Verify all 16 models are registered: cat datastore/models/registry.json | python -m json.tool
...
8. Check paper trading performance: python paper_trading/performance_report.py — report actual vs predicted returns honestly
9. Verify API docs accessible: curl http://localhost:8000/docs -o /dev/null -w "%{http_code}" — must return 200
...
2. Read the spec it references (look at the docstring for SPEC-ID)
...
1. Check docs/specs/08_specifications.md — does this already have a SPEC-ID?
2. If not: create a new SPEC-ID following the naming convention (SPEC-[CATEGORY]-[NNN])
3. Add the new spec to docs/specs/08_specifications.md in the right section
4. Add the corresponding test ID to docs/tests/09_automated_tests.md (SPEC-TRACE-001)
...
2. Run: python systems/ml_signal_engine/inference/train_all_phase[N].py --incremental
...
6. Test the fix in isolation: python [failing_module].py --date [DATE] --dry-run
7. Resume pipeline from checkpoint: python ingestion/scheduler/daily_pipeline.py --date [DATE] --resume
...
*Reference: docs/CLAUDE.md (master context) | docs/specs/08_specifications.md (all spec IDs)*
