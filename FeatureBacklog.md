...
| A103 | **Two index-history gaps in the supplied NSE CSVs** | Ingestion / Benchmarks | ⏳ | 2026-08-13: the 172 historical-PR CSVs ingested under A97 are missing two year-files. **Nifty Midcap 150 has no 2022 data at all** (0 rows for the whole calendar year, confirmed post-ingest) — any backtest window spanning 2022 benchmarked against it silently compares across a hole. The Nifty 50 2025 file is also absent from the CSV set, but that one is harmless: the daily scraper already covers 2025 (249 rows present). Fix is to download `NIFTY MIDCAP 150_Historical_PR_01012022to31122022.csv` from NSE and re-run `scripts/ingest_index_csv.py --glob 'NIFTY*_Historical_PR_*.csv'` — the ingest is idempotent (UPSERT on `(date, index_name)`, DO NOTHING by default) so a re-run costs nothing |
| A104 | **~30% of the new size-index history is NSE back-computation, not traded values** | Backtest / Benchmarks | ⏳ | 2026-08-13: NSE's PR exports publish an index BEFORE it launched by computing it retrospectively, with Close only and no Open/High/Low. The boundary is exact and verifiable — the first date with a non-null `open` is the real launch: Nifty Microcap 250 live 2022-01-10 (3,472 back-computed sessions), Nifty Midcap 150 / Smallcap 250 / Smallcap 50 live 2019-01-14 (2,731 each), Nifty Smallcap 100 live 2011-10-03 (928). Midcap 100, Midcap 50, Next 50 are live throughout. So a 2009-2026 backtest benchmarked against Nifty Microcap 250 is compared against fourteen years of retrospective computation. That is NSE's own arithmetic and defensible, but it is not the same claim as a series that traded, and excess-return figures over such a window should not be presented as equivalent. `config/benchmarks.py::IndexCoverage.comparison_caveat` surfaces the caveat and `/api/v1/indices` returns it; what remains is deciding the POLICY — whether a persona recommendation may rest on a back-computed comparison at all, and whether `is_live_over()` should gate the recommendation engine rather than merely annotate it |
...
| ID | Item | Area | Status | Blocked On |
|---|---|---|---|---|
| T13 | **Metric parity with Momentum in the primary Technical report** — rolling-window returns, YoY returns, churn/transactions per year, avg winner %, avg loser %, avg hold days and post-tax CAGR computed in-engine rather than post-hoc | Backtest (Technical) / Metrics | ⏳ | 2026-08-13: Technical's `metrics` block (`backtest/core/metrics.py`) carries none of these; they are reconstructed afterwards from trade-book CSVs in `backtest/ta_comprehensive_metrics.py`, so they are absent from `backtest_runs` and from the report JSON the frontend reads. **The semantics also disagree and must be reconciled, not just moved**: Technical's `rolling_returns` reports the median TOTAL return per window, Momentum's `rolling_window_summary` reports annualized CAGR — placing them in one table today compares incomparable numbers. Recommend annualized CAGR as the shared definition. Depends on A83 |
