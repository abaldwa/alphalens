...
| A83 | One **shared backtest report contract** — a single JSON schema every engine emits (setup / returns / consistency / risk / tradeQuality / income / equityCurve), with report generation moved out of `backtest/ta_comparison_report.py` and `scripts/run_momentum_dynamic_report.py` into a common `backtest/core/report.py` | Backtest / Reporting | ⏳ | 2026-08-13: scoped alongside the unified `/backtest-report/*` UI. Today Momentum's report rows carry `post_tax_cagr`, `churn_avg_transactions_per_year`, `avg_winner_return_pct`, `rolling_*_cagr` and income-mode fields that simply do not exist in the Technical `metrics` block, and Technical gets YoY/rolling/tax only post-hoc from trade-book CSVs. The frontend currently absorbs this divergence in per-channel adapters; A83 removes the need for them. Depends on A92 (registry) for strategy identity and ML40 for a single engine |
| A84 | Make the **point-in-time top-N-by-ADTV universe the default** for all channels, and wire the equivalent into `MomentumBacktester` | Backtest / Universe | ⏳ | 2026-08-13: **partially done.** `2b618a3c` added PIT ADTV ranking (`pit_adtv_top_n` -> `_build_config(top_n_by_adtv=)`) after finding the previous filter ranked on TODAY's universe CSV applied uniformly across 2009-2026 — lookahead bias, with INDOTECH the worked example (static rank 671, real trailing-21-session rank 1,554 on its entry date; it is in today's top 800 *because of* the +1,493.95% move the backtest claimed to capture). **Verified still open:** `pit_adtv_top_n` defaults to `None` in `run_orchestrator_backtest.py:560/854/1068`, so the old static behaviour is still what runs unless the flag is passed, and `MomentumBacktester` has no PIT equivalent at all. Target default: top 800 |
...
| A98 | **Split `regime_index_name` from `benchmark_index_name`; make the benchmark a per-strategy parameter** | Backtest / Benchmarks | ⏳ | 2026-08-13: `backtest/core/engine.py` uses ONE parameter (default "Nifty 500") for both regime detection and the benchmark equity curve (`_build_benchmark_curve`, line 944). Selecting a different comparison benchmark would therefore silently change which market regimes the strategy is allowed to trade in — altering the returns themselves, not just the comparison. Split them, add `benchmark_index_name` to the orchestrator config, queue-job schema, trigger API and the A92 registry definition, and persist it on the run so a report can state what it was compared against. Prerequisite for the UI benchmark selector to be more than a label. Related: A97 |
...
| A103 | **Enforce the readiness gate at the four generator call sites** — `ReadinessChecker` is built and tested but nothing calls it, so no signal is actually blocked today | Backtest / Signals | ⏳ | 2026-08-13: `backtest/core/readiness.py` (commit `36241ff4`) implements the full per-channel prerequisite contract — momentum needs `ohlcv_adjusted` through the as-of date; technical adds the feature panel plus every indicator the strategy's *registered entry predicates* reference (derived from `entry_criterion_json`, so the check cannot drift from the definition it guards); ML adds the model's feature set and a current artifact; fundamental adds PIT fundamentals plus derived ratios. It records blocked attempts to `signal_generation_blocked` naming the missing input. **What is missing is the call**: `BacktestOrchestrator.run()` and `PaperTradingRunner.propose_today()` never invoke it, so the gate is inert and a generator will still emit on partial data. The user's stated principle is "a delayed signal is better than a wrong signal" — execution is next market open either way, so there is hours of slack to spend on correctness. Partial generation must stay forbidden: a Technical strategy running on 60 of 66 indicators produces plausible WRONG signals, which is the exact failure this exists to prevent. Note `required_ohlcv_source='fyers'` is only enforced from `FYERS_COVERAGE_START` (2017-01-01); earlier rows are legacy with `source` NULL permanently, and enforcing it there would block every backtest date from the 2009 start to 2017 |
