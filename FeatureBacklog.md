...
| A64-followup | `tests/unit/test_schema.py::TestCreateSignalsSchema` still fails for `ml_multibagger` and `ml_signals` (same class of schema/doc drift as A64, different tables) | Data Layer / Tests | ⏳ | Found 2026-07-13 while fixing A64 — same failure class, e.g. `in_training_universe` present in the live DuckDB schema but not in the doc's/test's expected column set (per T11/T8/T12 BuildLog note). Not fixed here since A64's scope was specifically `ml_forensic`; needs its own side-by-side diff pass. |
...
| A67 | Sparkline column support — no sparkline rendering exists anywhere in the dashboard; needed for price/RS trend columns across tables (Sector Rotation, Signal Deep Dive, etc.) | Dashboard (all) | ⏳ | **2026-07-13, partial:** added a dependency-free `sparklineSvg()` helper (`dashboard/static/js/api.js`) and wired it into Sector Rotation's table (see ML28) on branch `feature/backlog-burn-a42-a63-a64-a67-a72-ml22-ml26-ml28-ml29-ml30-t9`. Signal Deep Dive and other tables mentioned in this item's scope not yet converted — left as a follow-up so the helper gets a second real consumer before calling the convention "framework-wide". |
...
| A72 | New cross-cutting Events table (corporate actions, bulk/block deals, 5d/21d/63d & MultiBagger recommendation triggers, forensic-flag dates) + chart overlay showing these as markers on `chart.html` | Data Layer / Dashboard (Technical) | ⏳ | `corporate_actions` and `bulk_deal_positions` tables already exist and can be reused for 2 of the 4 event types; recommendation-trigger and forensic-flag event rows are net-new; chart overlay itself does not exist at all today. **2026-07-13: skipped this pass** (attempted as part of the same combined-branch session as A67/ML28/T9/A63/A64) — 2 of the 4 event types are genuinely net-new (no existing table/query defines a "recommendation trigger" or "forensic-flag date" event, so their exact shape/dedup rules would be a design call, not a mechanical reuse), and the `chart.html` marker-overlay UI doesn't exist at all today, making this a multi-part feature bigger than a safe single-session addition alongside 10 other items. Left unimplemented rather than rush a half-verified overlay; good candidate for its own dedicated session. |
...
| ML22 | Merge Daily Insights and Daily WatchList screens — significant column/purpose overlap | Dashboard (ML) | ⏳ | Needs a design decision on which columns survive the merge (see ML-column-glossary note below). **2026-07-13: still skipped** — re-confirmed during the A67/A72/ML26/ML28/ML29/ML30 combined session that this genuinely needs the user's product call on which columns survive (the glossary narrows the candidate list but doesn't make the merge decision itself), not an auto-implementable non-intrusive change. |
...
| ML26 | Signal Deep Dive layout redesign: Forensic Score, MultiBagger Score, 52wk hi/lo up top; Recommendation History as paired Buy-date/Sell-date/Buy-price/Sell-price/CMP/rationale rows (collapsing a Buy that persists across N days into 1 row); per-horizon (5d/21d/63d) meta-label probabilities + range + Q50 return; SHAP explanation; all raw model scores moved to the bottom | Dashboard (ML) | ⏳ | Builds on ML8's existing "Recommendation History" section — needs the buy/sell-pairing aggregation logic, which doesn't exist yet. **2026-07-13: skipped this pass** — the buy/sell-pairing aggregation logic (collapsing a persisted N-day Buy signal into one paired row) is real new query logic with several edge cases (overlapping signals, re-entries, no matching Sell yet) that deserves its own focused implementation + tests rather than being squeezed into a combined 11-item session. |
...
| ML28 | Extend Sector Rotation (ML12) to 1d/5d/21d/63d relative-strength horizons with trend sparklines, ordered by market cap, with tickers as hyperlinks and %ages explained inline | Dashboard (ML) / Features | ⏳ | Extends `features/sector_rotation.py`, which currently only computes trailing-21-day RS (ML12 ✅). **2026-07-13, mostly done** on branch `feature/backlog-burn-a42-a63-a64-a67-a72-ml22-ml26-ml28-ml29-ml30-t9`: `compute_index_relative_strength()` now computes real `rs_1d`/`rs_5d`/`rs_21d`/`rs_63d` (horizons with insufficient real index_ohlcv history are `None`, never guessed) plus a rebased-close sparkline series per sector index and Nifty 500; exposed on `GET /api/v1/sector_rotation/report`; dashboard table (`dashboard/static/ml/js/sector_rotation.js`) now shows sortable RS-horizon columns, a 63d trend sparkline (A67), and tickers as hyperlinks + deep-dive icons (A69 convention) in the Top Stocks cell. 14/14 tests pass in `tests/unit/test_sector_rotation.py` (3 new). **Not done:** "ordered by market cap" — no per-sector market-cap aggregation exists anywhere in this codebase today, and inventing one (simple average vs. weighted by index constituent, which universe/fundamentals table to join) is a design call left for a follow-up rather than guessed here. |
| ML29 | Sector accumulation detection: (sum of each stock's delivery % × volume) / sector's total outstanding shares, tracked daily, to surface sectors under constant accumulation; drill-down by clicking a sector's %age | Features / Dashboard (ML) | ⏳ | Needs delivery-percentage and outstanding-shares data already ingested (bhavcopy delivery data) — net-new aggregation, no existing computation. **2026-07-13: skipped this pass** — confirmed during the combined A67/A72/ML22/ML26/ML28/ML30 session that this is genuinely net-new: no existing feature file aggregates delivery%×volume by sector, and "outstanding shares" isn't obviously a single already-joined column (shareholding-pattern filings vs. a dedicated outstanding-shares field) — which source to use for the denominator is a data-source decision, not a mechanical extension of existing code. |
| ML30 | MyHoldings: move off browser localStorage into a DB-backed table (ticker, purchase date, qty, sale date, sell price, purchase rationale, sell rationale, journal entry) with both manual entry and CSV upload | Data Layer / Dashboard (ML) | ⏳ | `holdings.html`/`js/holdings.js` already exist client-side-only (2026-07-11 exploration) — needs new schema + API routes, then swap the frontend's storage layer. **2026-07-13: skipped this pass** — scoped clearly enough to eventually auto-implement (new schema + CRUD API + swap the frontend storage layer, no design ambiguity), but it's a full new schema/table + multiple API routes + CSV-upload endpoint + frontend rewire, which is a meaningfully larger unit of work than the other items bundled into this session; deferred to its own session so it gets a complete schema-design pass + full CRUD test coverage rather than a rushed partial. |
...
| ML33 | Explore survival-curve output (mb_survival_*-style timing distribution, not just a binary hit/miss) for the 21d/63d gainer signal models, mirroring MultiBagger's RandomSurvivalForest approach | ML Signal Engine | ⏳ | 2026-07-13 investigation (experimental `_gainer` copy only, no production impact either way): feasibility confirmed — `training/labeling.py::compute_fixed_pct_labels` would need one addition (a `first_touch_day` field: the day index the +target_pct touch happened, NaN/censored-at-horizon if it never did) to produce the same `(duration, event)` pair survival models need, matching multibagger's `(duration_months, event)` shape. Assessed per-horizon value: low for 6d (window too short for day-level timing to change a trading decision), plausible for 21d/63d (a curve saying "likely to hit target by day 5" vs "likely by day 18, right at the window's edge" is real timing information the current binary classifier + quantile-magnitude head don't provide — useful for stop-loss/holding-period decisions). Cost check: unlike multibagger's crippling ~0.3% positive rate over 629K rows, 21d/63d have a much healthier ~26-35% positive rate (per Phase A results) over a far smaller chunked dataset, so RSF here should be cheap (minutes, not days) without needing the same negative-subsampling fix multibagger needed — though these models also retrain far more often in production than multibagger's occasional retrain, so even a cheap RSF is a small *recurring* cost, not a one-off. Architecturally cannot be "trained together" with multibagger — different data pipelines (`load_multibagger_training_data_from_db` vs `_build_gainer_training_dataset_chunked`), different label semantics, different model classes (`MultibaggerModel` vs `BaseSignalModel`-derived `GainerSignal21DModel`/`63DModel`) — would need its own small RSF head bolted onto the 21d/63d gainer models and its own training entry point, run sequentially (not concurrently) with any multibagger retrain to avoid resource contention, same pattern as the existing Phase A/B monitor chain. **Explicitly deferred**, not built this session — user chose to log it and stay focused on the MultiBagger relaunch |
