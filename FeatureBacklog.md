...
| ID | Item | Area | Status | Blocked On | Spec |
|---|---|---|---|---|
...
| A46 | Split `daily_pipeline.py`/`pipeline_scheduler.py` monoliths (1869/2488 lines) into per-concern modules | Scheduler | ✅ 2026-08-15 | 2026-08-15: **complete.** The 3,375-line `pipeline_scheduler.py` monolith is now five per-concern modules (`pipeline_run_lock.py`, `run_recording.py`, `pipeline_steps.py`, `pipeline_startup.py`, `scheduler_jobs.py`) behind a `pipeline_scheduler.py` facade that re-exports the full original surface, so every existing `from ingestion.scheduler.pipeline_scheduler import ...` consumer works unchanged (verified: `daily_pipeline.py`, `datastore/api/routers/ops.py`, `datastore/health/catchup.py`, `scripts/model_training_status.py`). The previous session's botched large-file rename+facade-replace was rebuilt clean-room from the known-good monolith. All 178 scheduler-related tests pass. Test files updated for the monkeypatch-reachability consequence of the split (facade attributes are re-exported bindings, so patching the facade no longer reaches a submodule's local `from ... import ...` binding — the 3 unit files + 2 model-training files now patch the owning submodule directly). This also closed a latent live-data hazard: `test_model_training_overdue_union.py`/`test_model_training_nightly.py` previously patched `pipeline_scheduler._trigger_model_retrain` by string path, which silently stopped reaching the submodule-local binding and would have invoked REAL training subprocesses; both now patch `scheduler_jobs._trigger_model_retrain`. Facade carries a `# TODO: delete after ML40-2.3` marker for when consumers can import the submodules directly. |
...
| A73 | Strategy queue (`run_strategy_queue.py`) re-fetches the full ~800-ticker OHLCV universe per job via independent HTTP calls to the DataStore API, repeated across all 432 queued jobs — real, redundant disk I/O amplification (API reads DuckDB per ticker per job) | Backtest / Data Layer | ✅ | 2026-07-25: flagged during a queue-reliability hardening session (resume-on-crash + systemd auto-resume + memory ceilings), not attempted — a shared OHLCV cache across jobs conflicts with the queue's deliberate subprocess-isolation-per-job design (`batch_common.py`'s module docstring: isolation is intentional, for memory-safety, not an oversight), so this needs its own scoped design (e.g. a pre-warmed on-disk Parquet snapshot per queue run, or a read-through cache keyed on (universe_spec, date range) shared read-only across subprocesses) rather than a quick patch. 2026-08-05: closed via `backtest/core/ohlcv_prewarm.py` — a Parquet+manifest snapshot keyed on (start_date, end_date) only (matching `get_ohlcv_bulk`'s actual dependency), prewarmed once by `run_strategy_queue.py::_maybe_prewarm_ohlcv` before launching any jobs when every orchestrator job in the queue shares one date range (the common technical-sweep shape), then read-through via `run_orchestrator_backtest.py`'s new `--ohlcv-snapshot-dir` flag (threaded into `_fetch_real_ohlcv`) — N subprocess jobs now make 1 live bulk fetch instead of N. Wired into `scripts/run_technical_recommended_strategies.py` (already at `max_workers=3`, `defer_db_writes=True`, `precomputed_matches_dir` — added `prefetch_feature_parquets=True` too) so the full 42-template technical sweep now runs as one optimized, persisted batch. New Technical > Batch Backtest menu page (`frontend/src/pages/technical/batch-backtest.tsx`) triggers this sweep and shows persisted `channel=technical` run history in one place |
| A76 | `scripts/feature_backfill.py`'s `--force` sentinel is written at the START of a run (right when `--force` is first honored), not after the run completes successfully — a crash/restart mid-run leaves the sentinel in place, so the restart falls back to skip-if-exists and can silently mark a year "done" without ever recomputing it | Scheduler / Feature Backfill | ⏳ | 2026-08-05: hit for real during the Fyers-primary feature recompute (commit 35f3fbb) — a 10-panel-worker attempt at year 2025 was killed mid-staging (before any per-date parquet was written), but its `--force` sentinel had already been written, so the very next restart (with the same `run_id`) saw the sentinel, fell back to skip-if-exists, found 2025's PRE-Fyers-switch parquets already on disk, and reported "year 2025 done" with 0 dates actually recomputed. Caught only by noticing the suspiciously-instant completion; fixed operationally that session by deleting the stale sentinel and re-running 2025 standalone with a fresh `run_id`, but the underlying script bug is unfixed — the sentinel should key off successful completion (e.g. written only after the final `Backfill complete` log line) or, better, track force-applied status per-date rather than per-run so a partial crash can't silently skip legitimate work on restart |
| A77 | Sweep the codebase for other places calling `.date()` on a value that may already be a `datetime.date` (not `datetime.datetime`) from a DuckDB query result | Data Layer / Scheduler | ⏳ | 2026-08-05: `ingestion/scheduler/daily_pipeline.py::step_download_fyers_daily` crashed on first real use — `MAX(date) FROM ohlcv_adjusted` against DuckDB's native `DATE` column type returns a plain `datetime.date` via `fetchone()`, not `datetime.datetime`, so the existing `.date()` call raised `AttributeError: 'datetime.date' object has no attribute 'date'`. Fixed at that one call site (`hasattr(x, "date")` normalization, commit 35f3fbb) but never swept for the same pattern elsewhere DuckDB DATE-column results are consumed — worth a `grep -rn "\.date()" ingestion/ datastore/ features/ scripts/` pass to check every hit is actually operating on a `datetime`, not a DuckDB-returned `date` |
| A78 | When building a shared fix (a helper/filter meant to replace a recurring bug pattern), grep every existing call site of what it replaces before considering the fix "done" — a fix wired into only the site that motivated it silently leaves the bug live everywhere else | Scheduler / Data Layer | ⏳ | 2026-08-05: `ingestion/scrapers/fyers_symbol_master.py::fetch_valid_nse_eq_tickers()` was built specifically to stop "Invalid Symbol Provided" FYERS API errors, and was correctly wired into `scripts/fyers_staged_backfill.py`. It was NOT wired into `ingestion/scheduler/daily_pipeline.py`'s `step_download_fyers_daily`/`step_fyers_health_check` (the daily-cutover counterpart of the same backfill), which kept hitting the exact error the filter was built to prevent, plus produced a false health-check failure (88.2% coverage vs. the 95% threshold, because ~270 permanently-invalid tickers were counted in the denominator). Caught only because the noisy per-ticker errors were visible in the log during a manual test run; fixed same session (commit 35f3fbb). No broader sweep done for other single-call-site fixes in this codebase that might have the same gap — this row exists as a process reminder, not a specific known-broken call site |
| A80 | `alphalens-scheduler.service` is currently stopped/disabled — `FYERS_PRIMARY_ENABLED=True` and the daily `step_download_fyers_daily`/`step_fyers_health_check` steps are wired in (commit 35f3fbb) but won't actually run until the scheduler is re-enabled | Scheduler / Ops | ⏳ | 2026-08-05: stopped mid-session to free the DuckDB write lock for a manual same-day Fyers injection, and deliberately left off afterward rather than re-enabled without explicit go-ahead (per this project's standing pattern of not silently re-enabling scheduler after an intentional stop). Also note: `ingestion/scrapers/fyers_login.py`'s unattended daily token refresh is permanently blocked by Cloudflare Turnstile on FYERS' login page (documented in that module's own docstring) — once the scheduler is back on, expect `fyers_login` job failures every day the cached token has expired; the manual OAuth auth-code exchange (`python3 -m ingestion.scrapers.fyers_backfill exchange <redirected-url>`) is the real daily dependency, not a bug to chase |
| A81 | Advanced-technical features (`hurst_exp_21d/63d`, wavelet/entropy/fracdiff/Lyapunov/RQA, 18 cols) are stored in TWO places with INCONSISTENT coverage, and neither is a superset of the other: (a) the daily feature Parquets `datastore/features/daily/*.parquet` — what `ScreenerEngine`/`TechnicalAdapter`/every backtest actually reads; (b) the `feature_panel_staging` table in `feature_panel_staging.duckdb` (18.9GB, 20.7M rows, 2007-2026), which carries all the same columns | Technical / Feature Store | ⏳ | 2026-08-08: found while verifying coverage before authoring the Category-T templates. Measured divergence, same feature same date: on 2016-08-08 the Parquet has 0 non-null `hurst_exp_63d` while staging has 63; staging holds pre-2020 values for only 86 distinct tickers (~3% of rows) so it does NOT rescue a 2016-start backtest either. Parquet coverage is 0% before 2021-08-06 (first populated date), then ~70% of ACTIVE tickers — the ~70% is BY DESIGN, not a gap: `scripts/backfill_advanced_technical_top_n.py` is deliberately scoped to the Top-N ADTV universe (default 800), whereas the daily pipeline path fills the full universe (99.8% of active tickers on 2026-08-07). Two consequences to resolve: (1) decide which store is authoritative and make the other derived or explicitly documented as an ML-panel-only artifact, rather than leaving two answers to the same question; (2) any strategy gated on these features silently screens a REDUCED universe on dates/tickers the backfill never reached — the screener treats a NULL feature as 'condition unmet' rather than raising (`engine.py::_apply_single_condition`), so this fails silently and understates signal counts instead of erroring. Related: A74 (same silent-NaN class of problem for the pattern-score templates), A75 (`hmm_*` never backfilled at all) |
| A82 | `compute_advanced_technical_features(all_rows=True)` is ~120.7s/ticker — a full-universe historical backfill is ~78h single-worker (~13-19h at 4-6 workers), which makes the advanced-technical history practically un-backfillable. Needs vectorization, not more cores | Technical / Feature Store | ⏳ | 2026-08-09: measured precisely (`compute 4 tickers: 483s -> 120.7s/ticker`; projection 2317 tickers 1 worker = 77.7h). Root cause is structural, not a deadlock or a stall — an earlier "deadlock" diagnosis was wrong (the pool is healthy: a real 2-worker/2-batch slice completed in 41s, and the long-running process was confirmed at 731% CPU, state `Rl`). The cost comes from `features/advanced_technical.py` recomputing all 17 features **per bar** (`for i in row_positions`, ~1,380 bars/ticker) instead of over rolling windows, compounded by `_run_pool_over_chunks` capping BLAS to 1 thread per worker. Fix direction: vectorize the per-bar loop (rolling/strided windows) so a backfill is hours not days — throwing workers at it is a linear win on a quadratic-ish problem and still costs a full day. NOT currently blocking the Category-T backtest: restricted to the top-800 ADTV universe the sweep actually screens (`max_tickers: 800`), `hurst_exp_63d` coverage is already 2021-08-06=71%, 2023-06-01=79%, 2025-08-14=91%, 2026-08-07=97% — the alarming "0%/~40%" figures elsewhere used the full 2,317-ticker universe as denominator, including ~1,500 illiquid names no backtest trades. Caveat that remains: T-templates gated on these features screen a modestly reduced universe in 2021-2022. Related: A81 (dual storage of the same columns), A74 |
| A83 | One **shared backtest report contract** — a single JSON schema every engine emits (setup / returns / consistency / risk / tradeQuality / income / equityCurve), with report generation moved out of `backtest/ta_comparison_report.py` and `scripts/run_momentum_dynamic_report.py` into a common `backtest/core/report.py` | Backtest / Reporting | 🔧 (contract exists; two legacy writers not yet migrated) | 2026-08-13: scoped alongside the unified `/backtest-report/*` UI. Today Momentum's report rows carry `post_tax_cagr`, `churn_avg_transactions_per_year`, `avg_winner_return_pct`, `rolling_*_cagr` and income-mode fields that simply do not exist in the Technical `metrics` block, and Technical gets YoY/rolling/tax only post-hoc from trade-book CSVs. The frontend currently absorbs this divergence in per-channel adapters; A83 removes the need for them. Depends on A92 (registry) for strategy identity and ML40 for a single engine **2026-08-14 audit of the 2026-08-13 commits: PARTIAL. `dd2cd94b` added `backtest/core/report.py::StrategyReport` — one shape for all four channels, with the rate-only and tax-basis conventions enforced in the contract. But A83's second half is NOT done: `backtest/ta_comparison_report.py` still owns `write_reports`/`build`, and `scripts/run_momentum_dynamic_report.py` still owns `run_dynamic_report`, so report generation has not moved into the common writer. Until Momentum's simulation is unified (ML40 Phase 2) the contract remains a schema translation over two genuinely different computations, exactly as this row's original note predicted.** |
| A84 | Make the **point-in-time top-N-by-ADTV universe the default** for all channels, and wire the equivalent into `MomentumBacktester` | Backtest / Universe | ✅ 2026-08-14 | 2026-08-13: **partially done.** `2b618a3c` added PIT ADTV ranking (`pit_adtv_top_n` -> `_build_config(top_n_by_adtv=)`) after finding the previous filter ranked on TODAY's universe CSV applied uniformly across 2009-2026 — lookahead bias, with INDOTECH the worked example (static rank 671, real trailing-21-session rank 1,554 on its entry date; it is in today's top 800 *because of* the +1,493.95% move the backtest claimed to capture). **Verified still open:** `pit_adtv_top_n` defaults to `None` in `run_orchestrator_backtest.py:560/854/1068`, so the old static behaviour is still what runs unless the flag is passed, and `MomentumBacktester` has no PIT equivalent at all. Target default: top 800 **2026-08-14 verified implemented (backlog audit): `DEFAULT_PIT_ADTV_TOP_N = 800` is now the default of both the `--pit-adtv-top-n` CLI flag and `run_orchestrator_backtest()`'s `pit_adtv_top_n` parameter (`backtest/run_orchestrator_backtest.py:105,1336,1655`), applied for every channel via `_build_config(top_n_by_adtv=...)`. Code is tagged `[A84, 2026-08-13]`.** |
| A85 | Make **circuit-lock fill refusal the default** (`block_circuit_fills=True`), and surface excluded candidates in the report | Backtest / Execution | ⏳ | 2026-08-13: **partially done.** `3bd69386` implemented per-bar fill refusal plus a chronic-locker rule, correctly separating "this price was not fillable" (a fact about a date) from "no price from this security is trustworthy" (a judgement about the ticker), and deliberately did NOT implement "exclude every share that hits circuit" — 2,134 of 3,159 tickers with real history have locked at least once, so that rule would discard 68% of the universe and preferentially the high-momentum names the Technical screens exist to find. **Verified still open:** `block_circuit_fills: bool = False` at `run_orchestrator_backtest.py:1070`, so no run excludes a locked fill unless the flag is passed; `circuit_band_pct` was `None` in all 195 unconstrained Technical runs to date. Also open: reporting the excluded candidates so the UI can show them |
| A86 | **Pre-tax and post-tax figures from a single run**, for every channel | Backtest / Tax | ✅ 2026-08-14 | 2026-08-13: **partially done.** `29b63d61` made tax a real per-FY cash outflow in the orchestrator (`StrategyPortfolio.apply_due_fy_tax`, called at `core/engine.py:762`), fixing a defect where the whole run's tax was subtracted once from closing equity so every rupee owed compounded for the life of the run — an interest-free government loan that CAGR, Sharpe and trade count all left invisible. **Verified still open:** Momentum still needs two full engine runs to produce both bases (`withhold_fy_tax=True` re-runs the whole backtest via `features/momentum_strategy.py::compute_fy_net_tax`), and Technical computes tax post-hoc on the trade book in `ta_comprehensive_metrics.py::tax_liability` rather than in-engine. Blocks the post-tax-headline UI from being truthful for Technical. Retire `backtest/momentum_tax.py`'s duplicate of `backtest/core/tax.py` **2026-08-14 verified implemented (backlog audit): `backtest/core/metrics.py` carries an explicit `tax_basis` field (`pre_tax`/`post_tax`) plus `reconstruct_pre_tax_curve()`, and `backtest/core/report.py` emits BOTH `cagr_pre_tax` and `cagr_post_tax` from one run. Code is tagged `[A86, 2026-08-13]`.** |
| A87 | **Separate signal generation from trade simulation, and parallelize** — generate a strategy's signals once, then simulate many variants against them | Backtest / Performance | ⏳ | 2026-08-13: the orchestrator already has the seam (`StrategyAdapter.generate_signals`, `core/engine.py:288`) and a file-based cache (`core/screener_cache.py` + `scripts/precompute_technical_screener_matches.py`), but reuse is per-file and per-run, not across variants in one process. `MomentumBacktester` fuses signal generation and simulation in one `run()` loop (the momentum panel is precomputed and injected, which is the partial equivalent). Becomes materially easier once A94's signal ledger exists — signals are then read from a table rather than recomputed. Note `exclusive_backtest_lock` currently prevents in-process concurrency of full orchestrator runs. **2026-08-14 measurement (this is the evidence A87 was missing):** phase instrumentation over 41 runs of the 186-job sweep shows the simulation is not the cost. Technical mean 247.8s wall clock, of which only 17.7s is measured inside `orchestrator.run()` and 138.6s is unattributed setup; momentum 82.8s of which 10.3s unattributed. A representative technical job: 162s total, 10.8s measured — `signals` 9.8s over 204 calls (48ms each), `universe` 0.77s, `finalize` 0.23s, everything else under 0.03s, `execution` 0 calls. Profiling the unattributed region directly: OHLCV fetch 39.5s, `_build_config` (ticker_dates + PIT ADTV panel) 12.7s, close pivot 0.7s, volume pivot 0.7s — all of it IDENTICAL across every job in the sweep, rebuilt in each of 186 subprocesses. The whole 4,275-day simulation costs ~11s. Note `defer_db_writes` deliberately disables `technical_screener_cache` (`run_orchestrator_backtest.py:1170`), so the 126 technical jobs also re-screen every date independently — the cache that exists for exactly this is off in the mode the parallel queue uses. User decision 2026-08-14: fix by sharing panels across strategies in one process (date-major, since both technical screening and momentum ranking are cross-sectional), before any further sweeps |
| A88 | **Income mode phase 2** — shortfall backfill and run-at-current-capital-until-profitable | Backtest / Portfolio | ✅ 2026-08-14 | 2026-08-13: **largely done for the orchestrator.** `57c565a8` added `top_up_after_loss` (`run_context.py:85`, default `True`): `True` refunds a losing year to base capital (the shortfall-backfill half), `False` injects nothing and the strategy must earn its way back (the run-till-profitable half). **Verified still open:** `MomentumBacktester`'s `annual_capital_reset_target` path (`momentum_backtest.py:893-921`) has no equivalent — it has only the withdrawal half — so the two engines' income modes are not comparable, and the unified report's Regular Returns mode reads null for momentum strategies **2026-08-14 audit of the 2026-08-13 commits: implemented by `6f8f5cc7`. Both 'regular returns' variants exist in `StrategyPortfolio` via `AnnualResetConfig.top_up_after_loss` (True = refund to base, False = run at current capital until it earns back), and `backtest/core/report.py::_income_from_ledger` now summarises the FY ledger into the report's income block, so the mode's result is actually readable.** |
| A89 | **Canonical `strategy_key` and display label emitted by the engines** (`{channel}:{variant_id}`), replacing the four current identity schemes | Backtest / Reporting | 🔧 (engines emit it; momentum key is a stopgap) | 2026-08-13: strategy identity is currently `variant_id` (momentum dynamic report), `strategy_id`+`run_id` (`backtest/strategy_id.py`), `template_name`+`exit_variant` (technical), and a localStorage `momentum_strategy_id` — the same strategy is labelled differently on the momentum hub (`STRATEGY_LABELS[v.strategy]`) than in the YoY matrix (`rowLabel()`). Momentum has no stable variant id field at all; identity is a dict key, a JSON sweep row, or a generated string. Subsumed by A92's `strategy_key` primary key; until then the frontend derives it client-side in `features/backtest-report/strategyKey.ts` **2026-08-14 audit: PARTIALLY implemented. The engine builds and emits `"{channel}:{name}"` (`backtest/core/engine.py:362`), `run_context.py:173` carries it as the canonical identity, and `report.py:276` prefers the engine-emitted key over a caller-supplied one. NOT closed because momentum's key is still a generated descriptor string rather than a registry row — see `backtest/run_orchestrator_backtest.py:525`, which states outright that it is a stopgap that goes away with ML40/ML41.** |
| A90 | **Equity curve (and benchmark curve) in every channel's report JSON** | Backtest / Reporting | ✅ 2026-08-14 | 2026-08-13: both engines compute one — `MomentumBacktestResult.equity_curve` (`momentum_backtest.py:1062`) and `StrategyPortfolio.equity_curve` serialized in `core/engine.py::_finalize` — but neither reaches the report JSON the frontend reads, so no report screen can draw one. Include `_build_benchmark_curve`'s output (see A98 for which index it should be) **2026-08-14 verified implemented (backlog audit): `BacktestRunResult.equity_curve` (`backtest/core/run_context.py:227`) is carried into `ReportContract.equity_curve`/`benchmark_curve` (`backtest/core/report.py:185,373`). The remaining `pending["equity_curve"]` marker only labels runs recorded BEFORE this landed, not a missing capability.** |
| A91 | **Channel-agnostic strategy deploy configs** so a Technical/Fundamental/ML strategy can be deployed from the backtest report | Backtest / Deploy | ✅ 2026-08-14 | 2026-08-13: `/api/v1/momentum/configs` and `pages/momentum/StrategyDeployPage.tsx` are momentum-only (`ConfigFormData` is a momentum parameter set), so the unified report's Deploy checkbox is necessarily disabled for the other three channels. Generalize the config schema and endpoints to carry `strategy_key` + channel + the shared setup attributes. Depends on A92 **2026-08-14 verified implemented (backlog audit): `datastore/schema/create_strategy_deployments.py` + `datastore/api/routers/deployments.py` (registered in `main.py:180`) deploy a registry strategy of any channel; `tests/unit/test_strategy_deployments.py` covers the Technical case that previously had nowhere to go.** |
| A92 | **`strategy_registry` table + loader** — one row per strategy across all four channels, carrying definition, entry criterion and exit criterion, append-only and PIT-versioned | Architecture / Strategy | ✅ 2026-08-14 | 2026-08-13: strategy definitions currently exist in four unrelated shapes — `systems/technical_analysis/screener/templates.py` (63 `ScreenerTemplate` dataclasses), `features/momentum_strategy.py::build_category_presets` (4 kwargs dicts), `features/fundamental_composites.py::STRATEGY_CATALOG` (26 entries) + `SCORE_FUNCTIONS`, and ML model classes; exit criteria are separate again (`EXIT_POLICY_VARIANTS`, 8 policy classes, `derived_exit_params.json`). `strategy_catalog` (`datastore/schema/create_strategy_catalog.py`) is the nearest existing thing but is populated FROM completed runs — a byproduct, not a declaration source — and should become a run-linkage view. Entry/exit use one predicate grammar for all channels, generalizing `ScreenerTemplate`'s `{"feature","op","value"}` dicts. **Versioning is non-negotiable**: definitions are append-only and runs record the version they executed, or editing a definition silently invalidates every historical result. Blocks A93-A95, T15, ML41, ML42, F7 **2026-08-14 verified implemented (backlog audit): `datastore/schema/create_strategy_registry.py` + `strategies/registry.py` loader + `strategies/migrations/` + `tests/quality/test_strategy_registry_invariants.py`. Table is created on every orchestrator run. NOTE: the per-channel MIGRATIONS remain open and are tracked separately (T15, F7, ML41, ML42).** |
| A93 | **`filter_registry` table + loader** — one row per filter concept, one implementation each | Architecture / Strategy | ✅ 2026-08-14 | 2026-08-13: the same seven filter concepts (ADTV floor, downtrend, circuit lock, HMM regime, quality gate, size/beta orthogonalization, chronic locker) are declared three times in three incompatible shapes — momentum as backtester kwargs in `scripts/run_momentum_filter_overlays.py`, technical as CLI/job-dict fields in `scripts/run_technical_filter_overlays.py::FILTERS`, fundamental hardcoded as `_PRESETS_NEEDING_LIQUIDITY_FLOOR`/`BESPOKE_PRESETS` inside the adapter. Existing shared pieces (`backtest/trade_filters.py`, `backtest/core/adtv.py`, `features/regime_signal.py`, `config/backtest_exclusions.py`) become the single implementation targets. Filter overlay sweeps then become registry queries instead of hand-maintained dicts. Blocked on A92. **2026-08-14: the three panel filters now have one implementation each** (`5c4e010a`, `99a120ca`). `backtest/adapters/panel_filters.py` holds `adtv_series`/`is_circuit_locked`/`downtrend_tickers`; `MomentumAdapter` delegates to it (72 existing tests pass unchanged, proving the extraction is behaviour-preserving) and `FundamentalAdapter` now applies them at all. It previously accepted NONE of the three and the orchestrator passed none on either construction path, so `--min-adtv-cr 1.0` on a fundamental run was recorded in `config_json` and rendered in the report while filtering nothing — measured across all 26 presets, control vs filtered gave 168 vs 168 trades with 0 buys added or removed. After the fix: 165 trades, 35 removed, 32 added, 8 of 11 trading presets changed. `disable_buys_in_regime` now RAISES for the fundamental channel rather than being silently ignored. The downtrend primitive subsequently moved to `features/momentum_signal.py` because calling `trailing_momentum_from_panel` from inside `backtest/` registers as a second momentum generator **2026-08-14 verified implemented (backlog audit): `filter_registry` schema (`datastore/schema/create_strategy_registry.py`), `strategies/migrations/filters.py` loader, and one implementation per filter concept in `backtest/adapters/panel_filters.py` (shared by Momentum + Fundamental). Covered by `tests/unit/test_strategy_migration_filters.py`.** |
| A94 | **`strategy_signals` ledger** — `(strategy_key, version, signal_date, ticker, action, ...)` written by backtest, paper and live alike | Architecture / Signals | ✅ 2026-08-14 | 2026-08-13: no table anywhere has this shape today. `ml_signals` is ML-only, `ta_signals` is alerts-only and is not even declared in `datastore/schema/create_signals.py` (created ad hoc by `daily_alert_checker.py`), `technical_screener_cache` is a compute cache keyed `(template_name, as_of_date, ticker)`, and Momentum and Fundamental persist no signals at all — so a live paper trade cannot be traced back to the signal that produced it. Two hard constraints: (a) row volume — 63 templates x ~800 tickers x ~4,300 sessions is order 1e8 rows if written naively, so persist only EMITTED signals, not every ticker evaluated, and partition by date; (b) the DuckDB single-writer lock is already the top operational failure mode, so writes must go through the existing `defer_db_writes` path rather than opening new concurrent writers. Prerequisite for A87 being real. Blocked on A92 **2026-08-14 verified implemented (backlog audit): `backtest/core/signal_ledger.py` writes the ledger, wired into `backtest/core/engine.py` behind `OrchestratorConfig.persist_signals` which defaults to **True**, and into `backtest/paper_trading/live_runner.py`. Covered by `tests/unit/test_signal_ledger_wiring.py` and `test_strategy_signals.py`.** |
| A95 | **Wire frontend, backend and backtest to the registries** — `/api/v1/strategies` + `/api/v1/filters`; backtest reads definitions instead of importing `TEMPLATES`/`build_category_presets`/`STRATEGY_CATALOG`; screener, alert checker and daily inference read the same rows | Architecture / Strategy | 🔧 (API + frontend done; backtest/screener still import definitions) | 2026-08-13: the end state that makes the registries load-bearing rather than a parallel copy — in particular the guarantee that a deployed strategy's definition and its backtested definition are the same row. Includes the `tests/quality/` guard test enforcing the AGENTS.md architectural invariants, which can only be switched on once all four channel migrations (T15, ML41, ML42, F7) are complete — it would fail immediately today. Blocked on A92, A93, A94 **2026-08-14 audit: PARTIALLY implemented. The backend half is live — `datastore/api/routers/strategies.py` serves `GET /api/v1/strategies`, `/api/v1/strategies/{key}` and `/api/v1/filters` off the two registries. The frontend half is NOT done: `grep -rn 'api/v1/strategies **2026-08-14, A95 frontend half LANDED + a real gap measured.** New `frontend/src/shared/api/strategies.ts` (typed client for `/api/v1/strategies`, `/strategies/{key}`, `/filters`) and `features/backtest-report/data/useStrategyDefinition.ts` (TanStack hook; version participates in the cache key so a run is never explained with a later revision's rules). `pages/backtest-report/strategy-detail.tsx` now renders a **Definition** card off the registry row, deliberately ALONGSIDE the existing run-derived Setup card rather than replacing it — when the two disagree that is a real finding (a run executed against a since-revised definition) and it is only visible because they come from different places. tsc clean, vite build passes. **MEASURED: 83 of 93 ledger strategy_keys resolve against strategy_registry; 10 do not, and all 10 are momentum** — the engine emits the generated stopgap descriptor (`momentum:mom_top10_12m_..._20260814`) while the registry holds real names (`momentum:all_risk_b1_1-50_lb3mo_weekly_top10`). So the Definition card is live for Technical/Fundamental/ML and degrades to an explicit 'no registry entry' message for Momentum. Closing that is ML41 + ML40 Phase 2 (identity from the registry row), exactly as `_momentum_descriptor`'s own docstring predicts. **STILL OPEN for A95:** (a) the BACKTEST still imports definitions directly rather than reading registry rows — `run_orchestrator_backtest.py:97` imports `TEMPLATE_STYLE`, `screener/engine.py:53` imports `TEMPLATES`, fundamental reads `STRATEGY_CATALOG`; (b) screener/alert-checker/daily-inference do not yet read the same rows; (c) the `tests/quality/` guard enforcing the AGENTS.md invariants stays off until T15/ML41/ML42/F7 land — note the registry is already POPULATED (3,219 rows: momentum 3,120 / technical 63 / fundamental 32 / ml 4), so those four are about CONSUMERS reading the rows, not about writing them. |api/v1/filters' frontend/src` returns nothing, so no screen consumes either endpoint yet.** |
| A96 | **Backtest window as a first-class parameter expressed in years**, plus sub-window metric recomputation | Backtest / API | ✅ 2026-08-14 | 2026-08-13: the window is set in four unrelated places with no shared default — `--start-date`/`--end-date` are `required=True` with no defaults (`run_orchestrator_backtest.py:1255`), queue JSONs carry literal dates, the API forwards them verbatim, and the frontend hardcodes `useState('2023-01-01')` (`pages/backtest/BacktestPage.tsx:884`). "N years" exists only as a `years_back: int = 10` local inside sweep scripts, never exposed in the API or UI. Add a window parameter (3y/5y/10y/15y/max or explicit dates) to the orchestrator, queue-job schema and trigger API, and support recomputing metrics over a sub-window of an existing run so the UI selector can do more than filter. Note any window starting before 2009-04-01 is unreliable (legacy->Fyers seam at 2007-04-02, and see A99-A102) **2026-08-14 audit of the 2026-08-13 commits: implemented by `b1ca86bb`. `--window 3y/5y/10y/15y/max` is a first-class CLI parameter resolved exclusive with `--start-date`, and `backtest/core/metrics.py::rolling_window_cagrs(window_years, step_months)` recomputes sub-window metrics.** |
| A97 | **Ingest the missing size indices into `index_ohlcv`** — Nifty Midcap 100, Nifty Midcap 150, Nifty Smallcap 250, Nifty Microcap 250 — and expose `GET /api/v1/indices` | Ingestion / Benchmarks | ⏳ | 2026-08-13: `TRACKED_INDICES` in `ingestion/scrapers/nse_indices.py` covers Nifty 50/100/500 plus 12 sector indices; the size indices are absent entirely, so a small/mid-cap rank-band strategy has no honest benchmark to be compared against. The whole path already exists — `index_ohlcv` table, `scripts/ingest_index_csv.py` (idempotent UPSERT on `(date, index_name)`, built for NSE's historical-PR CSV exports), `scripts/backfill_index_ohlcv.py`, and `HISTORICAL_INDEX_ALIASES` for CNX-era renames — so this is a data task, not a new pipeline. Record each index's true launch date rather than fabricating history before it (the existing file already declines to guess CNX-era names for Nifty Healthcare and Nifty Oil & Gas for exactly this reason). `GET /api/v1/indices` must return coverage start/end per index so the UI can disable an index that does not span the selected window |
| A98 | **Split `regime_index_name` from `benchmark_index_name`; make the benchmark a per-strategy parameter** | Backtest / Benchmarks | ✅ 2026-08-14 | 2026-08-13: `backtest/core/engine.py` uses ONE parameter (default "Nifty 500") for both regime detection and the benchmark equity curve (`_build_benchmark_curve`, line 944). Selecting a different comparison benchmark would therefore silently change which market regimes the strategy is allowed to trade in — altering the returns themselves, not just the comparison. Split them, add `benchmark_index_name` to the orchestrator config, queue-job schema, trigger API and the A92 registry definition, and persist it on the run so a report can state what it was compared against. Prerequisite for the UI benchmark selector to be more than a label. Related: A97 **2026-08-14 verified implemented (backlog audit): `BacktestOrchestrator.__init__` takes `benchmark_index_name` separately from `regime_index_name` (defaulting to it for back-compat) and every run records which index it used — `backtest/core/engine.py:664,729-738`. Covered by `tests/unit/test_benchmark_regime_split.py`.** |
| A99 | **~20 double-adjusted tickers (INFY, NCC, ZYDUSLIFE, MAHSEAMLES, TCI, EIHOTEL...) whose prior prices were adjusted twice** | Ingestion / Corporate Actions | ⏳ | 2026-08-13, surfaced by the legacy corporate-action repair: residual ex-date gap is roughly 1/expected rather than 1.0 — INFY's 2006-07-13 1:1 bonus leaves a 0.503 gap, meaning prior prices were halved twice. `scripts/repair_corporate_action_ratios.py` refuses these (`contradicted`) because `adj_factor` does not describe the stored prices, so the adjuster cannot rebuild them from `raw = current / adj_factor`. Needs the same validate-then-patch treatment as the RIGHTS one-off, via `scripts/validate_corporate_actions_fyers.py` against a trusted source. Pre-2017 history only, but INFY makes it material for any large-cap backtest starting before 2007 |
| A100 | **32 tickers blocked from corporate-action repair** | Ingestion / Corporate Actions | ⏳ | 2026-08-13: listed by `scripts/repair_corporate_action_ratios.py` (dry run). 12 are A99's double-adjusted set; the rest have one action already baked into the stored prices without being recorded in `adj_factor`, which makes the whole ticker unsafe to recompute. Also tracked here: 1,300 of 1,580 SPLIT/BONUS actions have no legacy price data either side of the ex-date and cannot be verified at all (mostly pre-2005 or delisted — harmless for backtests starting 2009, but it means the repair's coverage is bounded by price availability rather than by the classifier) |
| A101 | **Wire `find_discontinuities` into `data_integrity_check`** | Ingestion / Data Quality | ⏳ | 2026-08-13: the gate and its tests already exist (`ingestion/adjust/continuity_gate.py`); hooking it into the daily pipeline is the remaining step. It must **report rather than fail** the run for now — 1,706 legacy bars still exceed the 35% threshold post-repair (down from 1,814), so making it blocking today would fail every run. Revisit blocking-mode once that residue is characterised. Related: the 2026-08-05 `data_integrity_check` circular-gate lesson — do not add a check that blocks the very job that would fix it |
| A102 | **41 zero-ratio SPLITs remain** (down from 86) | Ingestion / Corporate Actions | ⏳ | 2026-08-13: these sit on A100's blocked tickers or lack the price data to confirm a recovered ratio. Note 4 zero-ratio BONUS records are **correct as-is** (bonus debentures and a DVR issue, which must never receive an equity adjustment) and must not be "fixed". See `ingestion/adjust/ratio_recovery.py` |
| A103 | **Enforce the readiness gate at the four generator call sites** — `ReadinessChecker` is built and tested but nothing calls it, so no signal is actually blocked today | Backtest / Signals | 🔧 (orchestrator + paper trading enforced) | 2026-08-13: `backtest/core/readiness.py` (commit `36241ff4`) implements the full per-channel prerequisite contract — momentum needs `ohlcv_adjusted` through the as-of date; technical adds the feature panel plus every indicator the strategy's *registered entry predicates* reference (derived from `entry_criterion_json`, so the check cannot drift from the definition it guards); ML adds the model's feature set and a current artifact; fundamental adds PIT fundamentals plus derived ratios. It records blocked attempts to `signal_generation_blocked` naming the missing input. **What is missing is the call**: `BacktestOrchestrator.run()` and `PaperTradingRunner.propose_today()` never invoke it, so the gate is inert and a generator will still emit on partial data. The user's stated principle is "a delayed signal is better than a wrong signal" — execution is next market open either way, so there is hours of slack to spend on correctness. Partial generation must stay forbidden: a Technical strategy running on 60 of 66 indicators produces plausible WRONG signals, which is the exact failure this exists to prevent. Note `required_ohlcv_source='fyers'` is only enforced from `FYERS_COVERAGE_START` (2017-01-01); earlier rows are legacy with `source` NULL permanently, and enforcing it there would block every backtest date from the 2009 start to 2017 **2026-08-14 audit: PARTIALLY implemented. `ReadinessChecker` is enforced at run start by `_enforce_run_readiness` (`backtest/core/engine.py:446,1018`) with `OrchestratorConfig.enforce_readiness` defaulting to **True**, and by `backtest/paper_trading/live_runner.py:210`. Not marked done because coverage of all four generator call sites is unconfirmed, and `run_orchestrator_backtest.py:1121` still force-disables it (`config.enforce_readiness = False`) on one path.** |
| A104 | **Channel-agnostic deploy page** — non-momentum strategies are selectable in the report but "Review & Deploy" still routes to the momentum-only `/momentum-deploy` | Backtest / Deploy | ⏳ | 2026-08-13: A91's backend (`strategy_deployments` + `/api/v1/deployments`, commit `99786004`) and frontend gate-lift (`36241ff4`) are both done — the Deploy checkbox is now enabled for all four channels and blocks only on `status='retired'`. But `features/backtest-report/ui/DeploySelection.tsx` still links the selection bar to `/momentum-deploy?prefill=…`, whose `ConfigFormData` is a momentum parameter set, so a selected Technical/ML/Fundamental strategy has nowhere to land. Needs a channel-agnostic page that collects the four genuine **deployment decisions** a backtest cannot supply — `initial_capital`, `start_date`, `sip_amount`, `portfolio_id` — and posts via `data/useCreateDeployment.ts`. The self-check already asserts those four are `unmapped` and absent from the draft payload, so no placeholder capital can be posted; the page must keep that true rather than defaulting them. Two follow-ups to fold in: adapters do not yet populate `strategyVersion`/`sourceRunId` on `StrategyReport` (the engine now resolves the real registry version via `resolve_strategy_version`, so the value exists — it just is not threaded through the report), and `/api/v1/deployments` does no validation on `capital_mode`, so the frontend's `lump_sum`→`lump` mapping would store a typo silently; add a server-side enum check |
...
| A108 | **One set of backtest signals per strategy per window** — a regenerated period must supersede the previous one, not accumulate beside it | Backtest / Signals | ✅ | 2026-08-18: **done.** `run_id` is part of `strategy_signals`' primary key, so re-running a strategy over dates it had already covered collided with nothing and simply doubled the rows — leaving "what did this strategy say on this date" with two answers and every ledger reader having to pick a run. New `strategies/signals.supersede_backtest_signals()` compares the new set against the prior one, reports the difference, then deletes the prior rows; `SignalLedgerRecorder.finalize()` (flush + supersede, in that order so the new set is durable before anything old is removed) replaces the bare `flush()` at `backtest/core/engine.py:1274`. Scope is deliberately **backtest-source only**: live and paper rows record what was actually acted on and are never superseded — that asymmetry is why this is a supersede pass rather than a narrower primary key. Keyed on `strategy_key` and NOT version, so a revised definition's stale rows for the same dates go too, with the change reported rather than absorbed. The comparison separates DECISION changes (date/ticker/action) from DETAIL changes (rank/size/conviction) because "picked different stocks" and "sized the same stocks differently" are different findings. 9 tests. The 136 pre-existing backtest-source ledger rows were purged at user request, since the strategies are being regenerated under new names |
| A109 | **The pre-commit mypy hook cannot pass on the core backtest modules** — 69 pre-existing errors across 8 files, so any commit touching them must bypass the hook | Tests / Tooling | ⏳ | 2026-08-18: found while committing A108. `pre-commit`'s mypy runs `--config-file=pyproject.toml` (stricter than a bare `mypy` invocation) over STAGED files only, so a legacy module is checked for the first time whenever someone happens to stage it — and then fails on debt it did not introduce. Verified on unmodified HEAD: `backtest/core/metrics.py`, `backtest/core/engine.py`, `backtest/momentum_metrics.py`, `backtest/momentum_tax.py`, `features/momentum_strategy.py`, `strategies/signals.py`, `features/momentum_signal.py` and `config/universe.py` produce **69 errors**, almost all `Missing type arguments for generic type "Dict"` and `Function is missing a type annotation`. Two consequences, both bad: a commit touching any of them must use `--no-verify` (which also skips ruff/flake8, the hooks that DO pass), and the count is unstable — fixing one error can unmask others that a prior error's `Any` was suppressing (observed: fixing one `arg-type` in `core/metrics.py` revealed 3 pre-existing bare-`Dict` errors on untouched lines). Fix is a typed-annotations pass per module, not a config relaxation — the hook is right, the code has debt. **RCA (2026-08-18):** `pyproject.toml` sets `strict = true` + `disallow_any_generics = true`, but enforcement is per-STAGED-FILE, so strictness is declared globally and applied incidentally — debt accumulates invisibly in every module nobody happens to touch, then lands on whoever next edits one. Repo-wide the real figure is **588 errors across 91 files** in `backtest/ strategies/ features/ config/` alone. **Cleared so far:** the 12 files touched by the A108/Phase-H work — `backtest/core/{engine,metrics,signal_ledger}.py`, `backtest/momentum_{metrics,tax}.py`, `features/momentum_{strategy,signal,live,universe}.py`, `strategies/signals.py`, `config/{universe,build_universe}.py`, `datastore/api/routers/momentum.py` — all now at zero, typing-only, no behaviour change. Clearing them surfaced TWO real bugs the type checker had been pointing at all along: A110 (annual reset on cost basis) and `calendar_cagr` returning a **complex** for a non-positive ending value from a function declared `Optional[float]`, while its sibling `trading_day_cagr` had always guarded it — both fixed. That is the argument for finishing the burn-down rather than relaxing the config. **Source fix SHIPPED 2026-08-18:** (a) `tests/quality/test_mypy_debt_does_not_grow.py` + `mypy_baseline.json` run mypy repo-wide (5.5s, cached) and pin per-file counts — a file may improve freely, but a regression or a new file with errors fails immediately, for whoever introduced it rather than for whoever next stages that module. Same shrink-only shape as `_UNCONVERTED` in `test_registry_is_load_bearing.py`, and verified to bite (a probe file with 2 errors fails the gate). A second test fails on STALE baseline entries so improvements get locked in rather than silently reversible. (b) **191 of the errors were an artifact of the config, not debt.** `follow_imports = "skip"` applied to `fastapi` made every `APIRouter` decorator resolve to `Any`, so EVERY route handler in EVERY router reported `untyped-decorator` no matter how well annotated — it had forced 28 `# type: ignore` comments into one router alone. A `follow_imports = "normal"` override for `fastapi` removed all 28 ignores and dropped the repo-wide count from **1,514 across 236 files to 1,323 across 221**. Remaining 1,323 are real debt to burn down module by module; the gate now stops it growing |
| A110 | **`capital_mode="annual_reset"` decided the FY withdrawal on COST BASIS, not mark-to-market** | Backtest / Core | ✅ | 2026-08-18: **found and fixed** while clearing A109's mypy errors — a `no-redef` warning on `prices` was the thread that led to it. `BacktestOrchestrator.run()` called `portfolio.apply_due_annual_reset(as_of, prices)` from the rebalance branch BEFORE that branch populated `prices`: only the non-rebalance branch filled the map, and it ends in `continue`, so on **every** rebalance date the reset received the empty dict initialised at the top of the iteration. Nothing raised, because `total_equity()` falls back to `pos.entry_price` for any ticker absent from the map — which is exactly what made it dangerous. The reset therefore valued open positions at COST rather than market, silently: a year of gains withdrew too little, a losing year topped up too little, and the call's own comment ("Needs prices (mark-to-market equity decides the withdrawal)") described behaviour it was not getting. Only `capital_mode="annual_reset"` runs are affected — `lump` and `sip` never call it. Fixed by moving the call below the rebalance branch's own price pass, still ahead of any sizing, which is the ordering constraint that matters. `tests/unit/test_annual_reset_uses_marked_prices.py` spies on the call and asserts it receives today's marked prices; **verified to FAIL on the pre-fix code** rather than merely passing after. Any published annual-reset result predates the fix and should be regenerated |
| A111 | **Burn down the remaining 1,319 mypy strict errors** — 218 files, tracked by the A109 baseline gate so they can only shrink | Tests / Tooling | ⏳ | 2026-08-18: split out of A109 once the gate was in place and the Phase-H modules were cleared. The gate stops the debt GROWING; this item is clearing it. **Distribution by package:** `systems` 361, `datastore` 252, `backtest` 246, `features` 222, `ingestion` 207, `strategies` 17, `config` 14. **By error class:** `type-arg` 493 (bare `Dict`/`List`/`set` with no parameters), `no-untyped-def` 289, `no-any-return` 133, `no-untyped-call` 103, `arg-type` 73, `assignment` 56, `union-attr` 45, `attr-defined` 28. The first two classes are ~59% of the total and are mechanical-but-careful: each one needs someone to READ what the structure actually holds, because blanket-`Any` satisfies the checker while destroying the value of having checked. **Why this is worth finishing rather than relaxing the config:** clearing the first 12 files surfaced two real defects the checker had been pointing at — A110 (annual reset deciding on cost basis) and `calendar_cagr` returning a complex from a function declared `Optional[float]`. The `arg-type`/`union-attr`/`attr-defined` classes (146 errors) are the ones most likely to hide more of those, so **do those first**, not the bulky `type-arg` ones. **Method that worked:** one agent per disjoint file-set, typing-only mandate, no `# type: ignore` without a written reason, verify with the file's own tests, and re-run the baseline gate at the end. Do NOT let a pass touch behaviour — every fix so far has been annotation-only, which is what made them safe to ship unreviewed line-by-line |
| A112 | **RANK_BANDS now partition rank 1-800 with no overlap** — bands 6/7/8 rebased to 201-300 / 301-500 / 501-800 | Features / Momentum | ✅ | 2026-08-18: **user-confirmed and done.** Ranks 200, 300 and 500 each belonged to TWO supposedly distinct band universes, so a boundary stock sat in two "separate" backtests and the two results were not the independent samples they appeared to be. (The same defect at rank 150 had already been fixed for bands 3/4; it recurring at three more boundaries is why it is now a test — `tests/unit/test_momentum_universe_bands.py` asserts contiguity, unique ids and full 1-800 coverage — rather than a comment asking someone to check.) **Identity consequence, handled:** strategy keys embed the boundaries (`variant_name` renders `..._b6_200-300_...`), so bands 6/7/8 resolve to NEW keys. The momentum migration was re-run: 720 rows registered (240 x 3), 0 revised, and a spot-check confirms `momentum:all_risk_b6_201-300_lb3mo_weekly_top10` and its band-7/8 equivalents resolve. Registry is now 3,120 momentum rows and the migration is idempotent on re-run. **Left for a decision:** the superseded rows are still `status='active'` — 240 each for `b6_200-300`, `b7_300-500`, `b8_500-800`, plus the pre-existing `b3_100-150`, `b4_150-200` and all 240 of band 5 (`100-200`, no longer in RANK_BANDS at all). That is **1,440 rows that are declared runnable but unreachable from RANK_BANDS**, the same governance shape A95-R1 closed for fundamental presets. Retiring them would make `status='active'` mean "runnable" again, but it also removes them from the A91 deploy surface, so it needs the user's call. Any published band-6/7/8 result predates the boundary change and covers a universe one stock wider than the current definition |
| A113 | **Phase A of UnifiedGeneratorRefactorPlan — make the live/backtest divergence visible and un-reintroducible** (A1 gate widening, A2 hardcoded-params gate, A3 parity harness) | Tests / Quality | ✅ | 2026-08-18: **done and mutation-validated.** **A1:** adding `datastore/` to `SCAN_DIRS` changed nothing on its own — nothing there constructs a `Signal()` or declares a `generate_signals` class — which confirms the real blindness was SHAPE, not scope: the gate only recognised generators written as classes, while every live divergence is a plain function. Three detectors added, each deleted by its scheduled fix: build-a-universe+score in one function (`momentum_live.compute_daily_ranking`, C2); any call to `matches_screener_preset` (fundamentals router, E2); re-deriving `score >= 1.0 - 1e-9` outside `ScreenerEngine` (`daily_alert_checker`, D3). **The gate found a divergence the plan had missed** — §1.2 named three `matches_screener_preset` sites, there are four: `get_fundamentals_pillar_summary`, the home page's "today's recommendation count" card, derives its number from the uncapped unranked rule. Also had to require BOTH halves of the momentum detector: a bare primitive-call rule flagged `routers/momentum.py:315`, which uses `trailing_momentum` only for a display column. **A2:** scoped to live paths only; the 8 `scripts/run_momentum_*.py` sites are deliberately unpoliced (a research runner pins its own experiment's parameters and deploys no capital — policing it would add permanently unremovable entries). Encodes one real distinction: `momentum_signal.LOOKBACK_MONTHS = [3,6,9,12]` is a MENU, not a chosen value, so containers are never flagged and scalars always are. **A3:** the harness CONTRADICTS §1.2's prediction and the reason is the finding. Momentum parity is EXACT for `all_risk` (Jaccard 1.000, 15/15, band 3 @ 2026-08-14) — correct, because `all_risk` is the unfiltered baseline (zero kwargs) and every adapter filter defaults to off, so both sides really are the same rule; now asserted as a hard invariant. The real gap is structural: `momentum_live.STRATEGIES` carries only band_id/label/rank_start/rank_end/strategy_id, so **3 of the registry's 4 categories are unrepresentable live** and a `balanced` or `max_defensive` strategy would run COMPLETELY UNFILTERED in production. **Critically, this is invisible to a diff today:** at the production floor (0.1cr, from `run_momentum_recommended_strategies.py:111` — NOT `settings.MIN_ADTV_CR`, which is 0.0 under the active profile) nothing is excluded; band 8's least liquid name trades 0.18cr. A diff-based C1 gate would pass today and keep passing until someone tightened a filter, so it is asserted structurally instead. An early harness draft used `settings.MIN_ADTV_CR` and reported flawless parity for a filter that filtered nothing — hence the sensitivity guard that raises the floor to 1000cr and asserts the selection moves, without which every parity pass here is vacuous |
| A114 | **C1 — `momentum_live` reads top_n/lookback_months/grace_cycles from `strategy_registry` instead of declaring them** | Features / Momentum | ✅ | 2026-08-18: **done, zero behaviour change.** The module constants applied ONE value to every rank band while the registry declares them per strategy, so two registry rows differing only in `top_n` were the same strategy live — and whichever had the approved backtest might not be the one that ran. Each of the 7 live strategies now carries a `registry_key` and reads `definition_json`. **Two things checked before writing code, both of which made this much safer than the plan's §5 risk table assumed:** (1) all 7 map onto existing registry rows whose declared values are IDENTICAL to the removed constants (top_n=15, lookback_months=6, grace_cycles=2), so no live pick moves — C1 changes where the answer comes from, not the answer; (2) `momentum_trades` and `momentum_contributions` are both EMPTY, and since the live `strategy_id` is persisted in them, remapping costs nothing now but would have been a data migration once real trades accumulated. The risk in Phase C is concentrated entirely in **C2**, not here. **One deliberate behaviour change:** `strategy_params()` raises `StrategyParamsUnavailable` when the registry has no row, or when a row declares only SOME of the three — there is no fallback default, because a fallback is exactly what made the constants dangerous (a run proceeds, looks healthy, trades parameters nobody approved). A partial row is rejected for the same reason: two real values plus one invented one is worse than none. `tests/quality/test_no_hardcoded_strategy_params.py` KNOWN_VIOLATIONS is now **empty** — the rule is absolute. 7 new contract tests; the 5 existing `test_momentum_live.py` tests that monkeypatched the constants now patch `strategy_params` (their need was always legitimate — a 29-day fixture cannot exercise a 6-month lookback — so only the injection point moved, onto the function production actually reads through) |
| A115 | **C2/C3 — momentum live path ranks through the shared `rank_universe` + `select_buy_pool`, and REFUSES a strategy whose declared filters it cannot apply** | Features / Momentum | ✅ | 2026-08-18: **live picks changed for bands 7 and 8, and the reason was NOT the filter chain.** `all_risk` declares no selection filters, so `select_buy_pool` is a no-op today and a zero diff was expected. Bands 1-4 and 6: 0 names changed. Band 7: **8 of 15**. Band 8: **6 of 15**. **Root cause — two implementations of 'trailing return' that were never the same function.** The conn-based `trailing_momentum` the live path used selects each ticker's last 126 rows via `ROW_NUMBER() OVER (PARTITION BY ticker ORDER BY date DESC)`; the backtested `trailing_momentum_from_panel` uses the shared calendar window. Identical for names that trade daily (hence bands 1-4/6 unchanged); for names with gaps the per-ticker window reaches further back in calendar time — AZAD 0.7351 vs 0.7058, max divergence **14 percentage points** — and it changed the eligible set too (196 scored vs 190 in band 7; DIACABS, E2E, GSPL, JSWDULUX, MTARTECH, STLTECH reach 126 rows only outside the panel window). So the pre-C2 live path ranked the illiquid bands on a window no backtest ever used, while the published band-7/8 results were measured on the calendar-aligned rule. **After C2, live selection equals MomentumAdapter's exactly for all 7 bands.** **C2's most important addition:** `_buy_pool_kwargs` maps registry-declared `filter_ids` onto `select_buy_pool` args and raises `StrategyNotRunnableLive` for any filter with no live data source (quality_gate, hmm_regime, size_beta_orthogonalized) rather than dropping it — dropping it is exactly the defect this phase removes. `adtv_capped_sizing` exempted because filter_registry types it `sizing`, not selection. **C3:** the parity harness now parametrizes over every band — band 3 alone reported perfect parity while 7 and 8 were diverging, and a test that cannot fail for the reason it exists is not evidence. The `live_selection_function` violation is deleted, not appeased. **Remaining momentum gap is a DATA gap:** 3 of 4 categories still cannot run live until quality-score/HMM-regime/market-cap-beta sources are wired — they now fail loudly instead of running unfiltered |
| A116 | **E1 — sector exclusion collapsed onto one owner, `is_sector_excluded(preset, sector)`** | Features / Fundamental | ✅ | 2026-08-18: the fundamentals router tested `preset in PRESET_EXCLUDED_SECTORS` and passed `sector` only then — a SECOND decision about whether an exclusion applies, alongside the predicate that already decides it. Passing sector unconditionally is behaviour-identical (empty set for a preset with no exclusions) and leaves one site that knows the rule; `/scores` and `FundamentalAdapter` now call the helper too. This is the duplication that let `/scores` skip the exclusion entirely and ship methodologically-invalid numbers to 4 frontend pages (caught in the 2026-07-28 review, by a human, not a test). **Finding that looks like a bug in the table but is not:** `PRESET_EXCLUDED_SECTORS` has 14 keys, `SCREENER_PRESETS` has 9 — the extra ones are composite-SCORE strategies (moat, sector_leader, qglp, longevity) that `matches_screener_preset` rejects with 'Unknown screener preset'. That is precisely why the shared owner is a helper answering the sector question alone rather than the predicate: `/scores` deals in score strategies, so routing it through the predicate was never possible. New AST gate keeps it collapsed (mutation-validated; a docstring mention is not mistaken for a read). mypy debt 1,320 → 1,319 |
| A117 | **B1 — live momentum signals dual-written into `strategy_signals` with `source='live'`** | Platform / Ledger | ✅ | 2026-08-18: `strategy_signals` is the ONLY table that ties a signal to the registry revision that produced it (`strategy_key` + `strategy_version`); `momentum_rankings` records a bare local `strategy_id` with no version, so a live pick could never be traced back to its declaration — the audit gap A94 exists to close. The ledger was 100% `source='backtest'` before this (1 row). `record_live_signals()` wired into the scheduler's `step_compute_momentum`; `momentum_rankings` writes unchanged (same additive pattern as `ml_dual_write.py`). **Two deliberate choices:** only SELECTED names are written as action='buy' — the full ranking is already in momentum_rankings, and a row per scored ticker per band per day is what turns this table from millions into hundreds of millions of rows (why `write_signals` refuses holds unless asked); and the ledger write is **non-fatal** — a failure logs loudly but does not take down the momentum step, since the rankings the dashboard serves are already committed and the ledger is an audit record, not the product. Tests use an isolated temp DuckDB; production ledger verified untouched after. **B2 is a no-op today** — there are no pre-existing live rows to backfill because nothing wrote any until now |
| A118 | **H3 — one measurement layer: every metric primitive has exactly one definition** | Tests / Quality | ✅ | 2026-08-18: new AST gate `tests/quality/test_one_measurement_layer.py`. A second Sharpe is a second answer to how a strategy performed, and the two that existed disagreed by sqrt(252/52) on weekly returns — 2.46 vs 1.12 — with neither looking wrong in isolation. `backtest/momentum_metrics.py` is tracked with 4 duplicate defs (cagr, sharpe_sortino_calmar, win_rate, rolling_window_summary), deleted by H4. **Re-exports are explicitly not duplicates** — momentum_metrics re-exports xirr/churn_factor from core after H1, and a gate punishing re-exports would punish the pattern that fixed the duplication. A third test pins `periods_per_year` existing and defaulting to None, because a hardcoded 252 is the defect that caused the duplication in the first place. Mutation-validated |

### Technical

| ID | Item | Area | Status | Blocked On | Spec |
|---|---|---|---|---|
| T13 | **Metric parity with Momentum in the primary Technical report** — rolling-window returns, YoY returns, churn/transactions per year, avg winner %, avg loser %, avg hold days and post-tax CAGR available from the engine rather than only from the post-hoc comparison report | Backtest (Technical) / Metrics | ✅ 2026-08-14 | 2026-08-13, **corrected**: an earlier version of this item claimed Technical had no rolling/YoY figures and that its rolling windows were median TOTAL return against Momentum's annualised CAGR. Both claims were wrong, and came from a summary that was not checked against the writer. `backtest/ta_comparison_report.py`'s `_rolling_windows` computes `((e1 / e0) ** (1 / years) - 1) * 100` — already an annualised rate — and `momentum_metrics.rolling_window_returns` returns `cagr_pct`. The two channels AGREE. (Note `ta_comprehensive_metrics.rolling_returns` is a different function that does emit totals, alongside a `median_annualized`; it is not what the comparison report writes.) What remains genuinely open: these metrics live only in the post-hoc comparison report built from trade-book CSVs, so they are absent from `backtest_runs` and from any run that has not had a comparison report generated — which is why the generic run adapter must mark them pending. Churn and post-tax CAGR are genuinely missing from both. Depends on A83 **2026-08-14 audit of the 2026-08-13 commits: implemented by `4dc92d26` ('Compute rolling windows, YoY, churn and win/loss in-engine'). `backtest/core/metrics.py` now emits `rolling_returns`, `yoy`, `churn_per_year` and `avg_winner_pct`/`avg_loser_pct` for EVERY orchestrator channel, so Technical no longer derives them post-hoc from trade-book CSVs under a different definition. Confirmed live in today's momentum run output.** |
| T14 | **Typed API responses for `/api/v1/technical_backtest/*`** | API (Technical) | ✅ 2026-08-14 | 2026-08-13: all six endpoints in `datastore/api/routers/technical_backtest.py` return bare `Dict[str, Any]` piped straight from the latest report file via `_read_latest_report(glob)`, unlike momentum's typed `DynamicReport`. The frontend adapter therefore has no contract to code against and breaks silently when a report's shape changes. Add Pydantic models in `datastore/api/schemas.py` **2026-08-14 audit of the 2026-08-13 commits: implemented by `94590f0e`. `/comparison` and `/trade_book` now declare `response_model=TAComparisonReportOut` / `TATradeBookOut` instead of bare `Dict[str, Any]` (`datastore/api/routers/technical_backtest.py:169,198`).** |
| T15 | **Migrate the Technical screener templates into `strategy_registry`** | Architecture / Technical | ✅ 2026-08-14 | 2026-08-13: move the 63 `ScreenerTemplate` dataclasses in `systems/technical_analysis/screener/templates.py` — plus `TEMPLATE_STYLE`/`STYLE_EXIT_PARAMS` and the `EXIT_POLICY_VARIANTS` x `backtest/config/derived_exit_params.json` exit parameters — into registry rows. **This migration should go first of the four**, because Technical's condition dicts are already `{"feature","op","value"}` with ops `lt **2026-08-14 audit of the 2026-08-13 commits: implemented by `045d770b`, which states 'With T15 already done'. `strategies/migrations/technical.py` (Owner: T15) declares every screener template as a registry row; 63 technical rows are live. Its own line 34 records that the screener/adapter still IMPORT `TEMPLATES` and that removing that is **A95's** job, not T15's — so the migration is complete and the consumer work is correctly tracked elsewhere.** |gt|lte|gte|eq|between|top_pct|bottom_pct`, so it validates A92's predicate grammar against real definitions before Momentum and Fundamental (whose rules are imperative Python) have to bend to it. Retain the import-time duplicate-screen gate (`_condition_signature`, `_KNOWN_DUPLICATE_GROUPS`) as a registry constraint. Blocked on A92 |
...
| A74 | 12 of 42 TA screener templates (D1, D3, D4, E3, E4, E7, F1, F3, F4, F5, F7, S006) depend on `flag_pattern_score`/`hurst_exp_21d`/`double_bottom_score`/`base_breakout_score`/`ichimoku_cloud_position`, which were 100% NaN before 2026-04-16 (feature only recently wired into the daily pipeline) — governance/mf_holdings/corp_action/fno/multibagger/real_economy_macro/deep_forensic backfill for 2016-2026 deliberately SKIPPED for this pass | Technical / Feature Backfill | ⏳ | 2026-07-31: added `matrix_builder.py::build_feature_matrix`'s `skip_slow_categories` param (threaded through `step_compute_features` and `scripts/feature_backfill.py --skip-slow-categories`) to backfill ONLY the 5 panel-staged categories (technical/intraday/pnd/advanced_technical/pattern_scores) the 12 templates need, since the full-category backfill measured ~400s/date (~12 days for 2016-2026) — governance/deep_forensic/real_economy_macro/multibagger dominate that cost and are NOT covered by `features/panel_staging.py`'s batch optimization. User explicit decision: address those 4 categories in a separate follow-up backfill pass, not this one. `taonly2016_*` run covers 2016-01-01→today; 2007-2015 still needs a second pass once this completes. Follow-up: either give governance/deep_forensic/real_economy_macro/multibagger the same batch-staging treatment `panel_staging.py` already gives the other 5 (real engineering, not just a flag), or accept the ~400s/date cost and schedule it as its own long-running unattended job like this one. |
| A75 | HMM regime features (`hmm_*` columns) never backfilled for the full universe/history — every backfill run so far (including A74's) has passed `--no-hmm` (~14x faster per date without it) | Technical / ML Signal Engine | ⏳ | 2026-07-31: explicitly deferred again while scoping A74's TA-template backfill. `compute_hmm_regime_features` fits one HMM model per ticker (stateful, not vectorized rolling-window math), so it can't reuse `panel_staging.py`'s batch-staging approach as-is — needs its own scoped design (e.g. a dedicated staged/chunked HMM fitting pass) before attempting a full 2007-2026 x 2,317-ticker backfill. Also blocks any model/screener template that reads `hmm_*` regime columns from returning real values on historical dates. 2026-08-05: gap widened again — the full 2017-2026 Fyers-primary feature recompute (commit 35f3fbb) also used `--no-hmm` for the same per-ticker-not-batchable speed reason (149s/date → 26s/date), so `hmm_*` is NaN for every date in that range too, on top of the pre-existing un-backfilled history. `scripts/backfill_hmm_regime.py` already exists as the intended fill-in mechanism but has not been run. |

### Fundamental

| ID | Item | Area | Status | Blocked On | Spec |
|---|---|---|---|---|
| F7 | **Migrate the Fundamental strategy definitions into `strategy_registry`, and persist fundamental signals** | Architecture / Fundamental | ✅ 2026-08-14 | 2026-08-13: move `features/fundamental_composites.py` — `SCREENER_PRESETS` (binary match rules), `STRATEGY_CATALOG` (the 26 strategies), `SCORE_FUNCTIONS` (ranking strategies), `PRESET_EXCLUDED_SECTORS` — plus the adapter-local `BESPOKE_PRESETS` and `_PRESETS_NEEDING_LIQUIDITY_FLOOR` in `backtest/adapters/fundamental_adapter.py` into registry rows. Fundamental persists no signals at all today, so it also needs an A94 writer. Blocked on A92/A93. Related: F4 (none of the 26 currently clear the benchmark-beat bar; 11/26 produce zero trades) — the registry does not fix that, but it makes the definitions inspectable, which is a precondition for diagnosing it **2026-08-14 audit of the 2026-08-13 commits: implemented by `045d770b`. `strategies/migrations/fundamental.py` (Owner: F7); 32 fundamental rows live.** |
...
| F4 | 26-strategy fundamental backtest (2026-07-29/30): none clear the benchmark-beat bar; 11/26 produce zero trades | Backtest / Features | 🔧 | Revisit after the missing `FUNDAMENTAL_FEATURES` columns (avg_roce_5y, delta_roce_3y, company_age_years, etc.) are backfilled — see writeup below |
...
| CA6 | POCL/JLHL: registered SPLIT ratio doesn't match the price factor actually applied in `ohlcv_adjusted` (2026-07-28 `data_integrity_check` spot findings) | Ingestion / Data Layer | ✅ 2026-08-01 | POCL: `corporate_actions` has SPLIT ratio=2.5 at ex_date=2026-07-21 ("Face Value Split... Rs 5 to Rs 2", which does math out to 2.5), but the check found the OHLCV's implied price factor matches a BONUS ratio of 1.5 instead (0.00% diff — an exact match, not noise). JLHL: SPLIT ratio=5.0 at ex_date=2026-07-24 ("Rs 10 to Rs 2", also mathematically correct as registered), but implied price factor matches BONUS ratio=0.333 (566.67% diff). User explicit decision: rather than resolve which side (registered ratio vs. applied adjustment) was wrong, sidestep it — pull latest raw OHLCV from Fyers for both tickers with adj_factor forced to 1.0, same treatment as the 6 spot_check tickers (`scripts/fyers_force_overwrite_tickers.py`, queued in the 2026-08-01 Fyers correction chain). The underlying `corporate_actions` row is left as-is (not edited) — only the OHLCV rows were overwritten. |

### Machine Learning

| ID | Item | Area | Status | Blocked On | Spec |
|---|---|---|---|---|
| ML40 | **Complete the Momentum -> Orchestrator consolidation** so Momentum and Technical genuinely share one engine, one metric module and one report writer | Backtest / Momentum | 🔧 (Phase 1 landed 2026-08-14; Phase 2 = retire MomentumBacktester) | 2026-08-13: `backtest/run_strategy_queue.py:85` records that "Momentum engine consolidation Phase 2" is underway (`rank_band_id` added, `backtest/adapters/momentum_adapter.py` exists), but the momentum dynamic report still runs the standalone `MomentumBacktester` (`backtest/momentum_backtest.py`, in-process, no DB), which fuses signal generation with simulation and has its own metrics (`momentum_metrics.py`) and its own tax module (`momentum_tax.py`). Until this lands, A83's shared report contract is a schema translation layer over two genuinely different computations rather than one report writer. Also the reason A84/A86/A88's orchestrator-side fixes did not reach Momentum. **2026-08-14 progress and a correction.** (a) Exit cadence: the orchestrator ran its exit policy every trading day for every channel, but momentum's entries exist ONLY on rebalance dates, so a position stopped out on day 3 of a 63-day cadence left its slot empty for 60 days. Measured at top_n=10: lb6 held a MEDIAN of 1 position, lb3 a mean of 4.09. `OrchestratorConfig.exit_policy_cadence` fixes it (`e0a503e0`); lb3 control then holds a median of exactly 10 and CAGR moves 1.25%->6.69%, lb6 0.89%->8.82%. Every momentum number produced before that commit is understated. (b) `grace_cycles` was hardcoded at 2 with no CLI flag or queue field, so none of the 500+ runs to date ever varied it; now wired through both construction sites and recorded in `config_json` (`a7ec80a3`). (c) `atr_adaptive` is NOT adaptive under the orchestrator: `entry_atr_pct` is only ever populated by the legacy `backtest/engine.py` (hardcoded `pillar="technical"`), so `core/engine.py` always passes `atr_pct=NaN` and every position falls back to flat TARGET_PCT=0.15 / STOP_PCT=-0.075. This affects all three orchestrator channels. (d) **The consolidation is still the blocker for the real work**: `MomentumBacktester` remains the only place the per-band design lives — 7 rank bands x 4 category variants x 4 lookbacks x 5 rebalance periods x 3 top_n — including `exit_rank`, `trailing_stop_pct`, `investable_pct` and `max_pct_of_adtv`, none of which `MomentumAdapter` has. It is the one remaining tracked violation in `tests/quality/test_one_generator_per_channel.py` **2026-08-14, ML40 Phase 1 LANDED (the one-engine consolidation).** `MomentumBacktester.run()` and `MomentumAdapter.generate_signals()` no longer rank or filter independently: both now call the same `features/momentum_strategy.py` functions -- NEW `rank_universe` (the ranking, including the momentum_panel fast path), NEW `keep_set_for_exit` (the asymmetric exit band), plus the existing `select_buy_pool`, `decide_grace_transitions` and `sticky_promoted_holdings`, which the adapter had been duplicating rather than calling since 2026-08-09. `backtest/momentum_backtest.py` now imports NO momentum primitive at all, so the `duplicate_momentum_ranking` entry in `tests/quality/test_one_generator_per_channel.py` is RESOLVED and deleted (the gate reports zero offenders, it was not appeased). `exit_rank` is ported onto `MomentumAdapter` and wired end-to-end -- `--exit-rank` CLI, `_ORCHESTRATOR_FLAGS` (so both queue runners can sweep it), and the `_momentum_descriptor` identity string (`xr{n}`), so a control and an exit-band run can no longer collide in `strategy_signals`. Verified on real data (2020-2024, top200, lb6): control CAGR 0.416/44 trades vs `--exit-rank 20` CAGR 0.468/41 trades -- fewer trades, winners ridden longer, the intended effect. 296 tests green including all of `tests/quality/`. **TWO REAL BUGS surfaced by the consolidation, both fixed:** (a) `select_buy_pool`'s ADTV floor lacked `.reindex(pool.index)`, so a ticker missing from the volume panel was absent from the Series rather than NaN in it, never matched `isna()`, and was silently treated as LIQUID -- the adapter had the reindex and was correct, MomentumBacktester did not; this changes results for any MomentumBacktester run that supplied a volume panel AND set `min_adtv_cr`. The no-volume-panel case is now explicitly separated and still a no-op, preserving that documented contract. (b) `momentum_strategy.is_circuit_locked` lacked the `pos >= len(idx)` bound check its `panel_filters` twin had, so it raised IndexError where the other returned False; the safer form won and `panel_filters` now delegates rather than keeping a second copy of `adtv_cr`/`is_circuit_locked`. **REMAINING (Phase 2), deliberately not attempted here:** `MomentumBacktester` the CLASS still exists -- ~15 scripts plus `systems/copilot/backtest_bridge.py` construct it. Its simulation loop (cash, positions, FY tax, capital reset) is still separate from `core/portfolio.py`, and `momentum_metrics.py`/`momentum_tax.py` are still separate from `core/metrics.py`/`core/tax.py` -- so A83's shared report contract is still a translation layer over two computations. The other three knobs are NOT adapter-level and were left alone rather than half-wired: `trailing_stop_pct` is a DAILY exit but the adapter is only called on rebalance dates (needs orchestrator support, and note `exit_policy_cadence="rebalance"` now suppresses daily exits for momentum); `investable_pct` is portfolio sizing; `max_pct_of_adtv` already exists generically as `StrategyPortfolio.adtv_cap_fraction` (default 0.10) and needs reconciling with momentum's per-run value rather than a second cap. |
| ML41 | **Migrate the Momentum strategy definitions into `strategy_registry`** | Architecture / Momentum | ✅ 2026-08-14 | 2026-08-13: move `features/momentum_strategy.py::build_category_presets` (the four cumulative kwargs dicts — `all_risk` -> +liquidity/ADTV/circuit -> +regime disable -> +size/beta orthogonalization) and the sweep grids currently scattered across seven `scripts/run_momentum_*.py` files into registry rows. Momentum has no stable variant identity today — it is a dict key, a JSON sweep row (`filter`/`band`/`lookback_months`/`rebalance`/`top_n`), or a `backtest/strategy_id.py` generated string — so this migration is what finally gives it one (A89). Blocked on A92/A93 **2026-08-14 audit of the 2026-08-13 commits: implemented by `045d770b` ('MOMENTUM (ML41) — no named strategies existed at all'). `strategies/migrations/momentum.py` turns the four cumulative filter presets and the sweep grid into declarative rows; 3,120 momentum rows are live. NOTE the remaining gap is IDENTITY, not migration: momentum RUNS still emit the generated `_momentum_descriptor` key, so 10 of 93 ledger keys do not resolve against these rows — tracked under ML40 Phase 2 / A95, not here.** |
| ML42 | **Migrate the ML models into `strategy_registry` and reconcile `ml_signals` with the `strategy_signals` ledger** | Architecture / ML | ✅ 2026-08-14 | 2026-08-13: represent the signal models (`signal_5d/21d/63d`, `MetaLabeler`, the exit models) as registry rows keyed to the trained artifacts in `datastore/models/registry.json`. Open question to settle as part of this: `backtest/adapters/ml_adapter.py` deliberately does NOT implement the `StrategyAdapter.generate_signals()` protocol (it wraps the frozen `backtest/engine.py` as a black box) — decide whether that exemption survives A87 and A94, or whether ML must emit signals through the same seam as everyone else. Blocked on A92/A94 **2026-08-14 audit of the 2026-08-13 commits: implemented by `045d770b`. `strategies/migrations/ml.py` (Owner: ML42); 4 ml rows live.** |
...
| ML39 | Wire ML backtests (`kind: "iterative_retrain"` jobs) into the same auto-resume + reboot-persistent + completion-popup queue infrastructure now covering Technical/Fundamental/Momentum | ML Signal Engine / Backtest | ⏳ | 2026-07-25: user explicitly deferred ("we will address ML separately") while building the queue-reliability stack (resume-aware `run_strategy_queue.py`, `alphalens-backtest-queue@.service` systemd template, `exclusive_backtest_lock` sequential-execution guarantee, memory ceilings). The plumbing is already channel-agnostic and ready — no ML-specific queue definition file exists yet at `backtest/reports/queue_defs/`; needs an actual ML queue (`kind: "iterative_retrain"` jobs, one per horizon/variant to retrain) built and handed to `systemctl --user enable --now alphalens-backtest-queue@<suffix>.service` the same way the current Technical/Fundamental/Momentum queue was |
...
| REV11 | `fundamentals_history` schema drift risk: the table is created once via `SELECT * FROM fundamentals WHERE 1=0` and never re-synced when `fundamentals` later gains a column via `_migrate_added_columns` (which has happened repeatedly per the schema file's own history) — the next fundamentals column addition will make `append_fundamentals_history`'s `SELECT *` insert fail with a column-count mismatch, and that failure is **not caught** in `write_fundamentals`/`write_fundamentals_batch`, so it 500s the whole write endpoint (with the primary `fundamentals` upsert already committed, history silently missing) | DataStore API | ✅ 2026-07-21 | Fixed both halves: (1) new `_sync_fundamentals_history_columns()` (`datastore/schema/create_normalised.py`) diffs `information_schema.columns` between `fundamentals`/`fundamentals_history` and `ALTER TABLE ... ADD COLUMN` for whatever's missing, in `fundamentals`'s own column order (so `append_fundamentals_history`'s positional `f.*` insert stays aligned) — wired into `create_schema()` right after `_migrate_added_columns`. (2) Both `append_fundamentals_history(...)` call sites in `datastore/api/routers/fundamentals.py` now wrap the call in `try/except Exception: logger.exception(...)` — the primary upsert already committed, so a history-append failure no longer 500s the write. Tests: `tests/unit/test_schema.py::test_fundamentals_history_column_sync_self_heals`, `tests/unit/test_fundamentals_history.py::TestWriteEndpointSurvivesHistoryAppendFailure` (2 tests) |
| REV12 | Six-plus `technical.py` router endpoints (alerts/watchlist/consensus/strategy-history) wrap their DuckDB query in a bare `except Exception: return <empty response, count=0>` — a real infrastructure failure (broken lock, malformed query, schema mismatch) is indistinguishable from a legitimate "nothing happened today" and presents as a clean, empty dashboard | DataStore API | ✅ 2026-07-21 | Narrowed all 7 `except Exception` sites in `datastore/api/routers/technical.py` to `except duckdb.Error` — a real duckdb-layer failure still returns the existing empty response, but any other exception type now propagates to FastAPI's default 500 instead of hiding as a clean dashboard. Test: `tests/unit/test_technical_router.py::test_unexpected_non_duckdb_exception_propagates_as_500` |
...
| REV15 | `sector`/`tier` columns exposed by `build_universe_csv`/`build_full_nse_universe_from_db` reflect NSE's *current* index/sector snapshot, not point-in-time membership — any feature or backtest conditioning on `sector`/`tier` across a multi-year window is implicitly applying today's classification retroactively (classic label look-ahead), acknowledged in the code's own comments but not yet mitigated the way the momentum-specific market-cap-rank proxy mitigates the tier case | Data Layer / ML Rigor | ✅ (guardrail) 2026-07-21 | A full PIT-joined sector/tier history is a separate data-ingestion project, out of scope; implemented the review's own cheaper accepted alternative instead — new non-critical `check_11_sector_tier_lookahead` (`backtest/integrity_checker.py`) fails (warns) when a `feature_df` carries a `sector`/`tier`/`market_cap_tier` column AND its date range spans more than one calendar year, the exact scenario where today's classification applied retroactively becomes a real risk. Tests: 3 new cases in `tests/unit/test_backtester.py` (multi-year fails, single-year passes, column-absent passes) |
| REV16 | `features/sector_accumulation.py`'s per-day accumulation score silently drops any ticker missing PIT `shares_outstanding`/`volume`/`delivery_pct` from the sector sum with no floor on `n_stocks_included` — a sector where most constituents lack data can produce a plausible-looking but misleading score with no visible warning | Features | ✅ 2026-07-21 | `compute_sector_accumulation` now also returns `n_stocks_total_in_sector` (real sector constituent count from `config.universe.load_universe()`, regardless of data availability) and a derived `low_coverage` flag (`n_stocks_included < 50%` of `n_stocks_total_in_sector`); both threaded onto `SectorAccumulationRow` in the router so the dashboard can visibly flag a low-coverage score instead of it looking uniformly trustworthy. Tests: 2 new cases in `tests/unit/test_sector_accumulation.py` (low/high coverage) plus a router-level assertion |
| REV17 | Same same-day signal-and-execution convention (`generate_signals(as_of)` then fill at `price_lookup(ticker, as_of)`, i.e. that day's own close) is used throughout `backtest/core/engine.py`/`backtest/adapters/technical_adapter.py` with no integrity check flagging it and no next-day-open variant to quantify the overstated fill quality it implies | Backtest | ✅ 2026-07-21 | Made the convention an explicit, tested config choice instead of a silent one: new `OrchestratorConfig.execution_timing: Literal["same_day_close", "next_day_open"]` (`backtest/core/engine.py`, default `"same_day_close"` — zero behavior change for existing callers). `"next_day_open"` fills buy/sell signals at the next trading day's `price_lookup` value instead of the signal day's (falls back to same-day with a logged `DataGap` at the last rebalance date, where no later day exists); `BacktestRunResult.execution_timing` now records which convention produced every run. Threaded through `WalkForwardRunner` too. Tests: 4 new cases in `tests/unit/test_core_engine.py` (default recorded, next-day fill actually used, last-rebalance fallback logged, missing-next-day-price data gap) |
| REV18 | `backtest/integrity_checker.py::check_04_survivorship` only checks that the delisted-ticker set is *non-empty*, not that it's a plausible fraction of history — a near-complete universe missing 1-2 delisted names passes this "critical" check while still carrying real survivorship bias | Backtest | ✅ 2026-07-21 | Added a `min_delisted_ratio: float = 0.01` field to `BacktestIntegrityChecker`; `check_04_survivorship` now fails when `len(delisted)/len(historical) < 1%`, not just when the delisted set is empty. Existing tests (1/3, 1/2 ratios) stay well above the floor and are unaffected. Tests: 2 new cases in `tests/unit/test_backtester.py` (implausibly-low ratio fails, plausible ratio passes) |
| REV19 | `sortino_ratio`/`calmar_ratio` (`backtest/core/metrics.py`) silently return `None` on degenerate inputs (zero downside periods, `mdd==0`) with no visibility into how often this happens across folds, so "genuinely excellent, no drawdown" and "too few observations to compute a real ratio" are indistinguishable in reports | Backtest | ✅ 2026-07-21 | Both functions now return `(value, none_reason)` tuples — reasons: `insufficient_returns`/`no_downside_periods`/`zero_downside_std` (Sortino), `no_cagr`/`zero_or_undefined_drawdown` (Calmar). `BacktestMetrics` gained `sortino_none_reason`/`calmar_none_reason` fields so any consumer aggregating many runs can compute a real None-rate/reason breakdown instead of guessing. (`backtest/core/metrics.py` computes once per run, not per-fold — no fold-aggregation machinery existed to build on, so fixed at the source instead.) Tests: extended `tests/unit/test_core_metrics.py` with a case per reason code |
...
| REV26 | Same NaN→Pydantic-float risk (finding REV25's class) not yet checked/fixed in `big_investors.py`, `holdings.py`, `momentum.py`, `valuation.py`, `watchlist.py`, `copilot.py` — flagged by the adversarial review but not individually verified this session | DataStore API | ✅ 2026-07-21 | Audited all 6 (via grep for `fetchdf`/`.to_dict(`): `big_investors.py`, `holdings.py`, `momentum.py`, `valuation.py`, `copilot.py` build responses only from `.fetchall()` tuples or in-memory dataclasses, where DuckDB NULLs already surface as Python `None`, not NaN — no fix needed. `watchlist.py`'s `/daily` endpoint was the one real `fetchdf()`-sourced path (`price_map`); extracted a `_build_price_map()` helper with an explicit `pd.notna()` guard (belt-and-suspenders — `ohlcv_adjusted.close` is schema-NOT-NULL today, so this isn't currently reachable via that table, but matches the same cheap always-correct pattern used in `fundamentals.py`/`sector_accumulation.py`). Tests: `tests/unit/test_watchlist_daily_router.py::TestBuildPriceMap` (2 tests, direct DataFrame-level unit tests since the schema constraint makes an end-to-end NaN unreachable) |
| REV27 | DuckDB read/write lock race (`datastore/api/db.py`) is time-bounded (≈3.5s retry-with-backoff), not eliminated — any write step (full backfill, universe-wide `compute_features`) holding the write lock longer than that will still hard-fail a concurrent API read, the same failure class that already crashed the scheduler once (per project memory) | DataStore API / Ops | ✅ 2026-07-21 | Moved `DUCKDB_LOCK_RETRY_ATTEMPTS`/`DUCKDB_LOCK_RETRY_BASE_DELAY_S` from hardcoded locals in `db.py` to `config/settings.py` (env-overridable), default attempts raised 4 → 6 (worst case ~3.5s → ~15.5s) so an operator can extend the budget for a known-long write without a code change; added an explicit operational-rule note (avoid starting a long write while API traffic is expected — this retry is a bounded mitigation, not a guarantee). Tests: new `tests/unit/test_db_lock_retry.py` (5 tests: settings sourced correctly, default raised to 6, retries-then-succeeds, gives-up-after-exhausted, non-lock IOException doesn't retry) |
...
### F4 — 26-strategy fundamental backtest run: no strategy currently clears the bar (2026-07-29/30)

**Background.** Following the vectorized governance/mf_holdings/corp_action
panel rewrite (see below) and a `FundamentalAdapter` extension adding a
3rd "composite-score" signal branch (rank the universe by
`SCORE_FUNCTIONS[preset](ratios)`, take top N — mirroring
`MomentumAdapter`'s ranking idiom) plus missing
`FUNDAMENTAL_PRESET_DEFAULT_HORIZON`/`FUNDAMENTAL_PRESET_EXIT_PARAMS`
entries for 16 previously-unqueueable strategies (commit `7f45fc8`), all
26 `STRATEGY_CATALOG` fundamental strategies were run through
`run_strategy_queue.py` (`backtest/queues/queue_20260729_fundamental_26strategies.json`,
`top_n=10`, `universe_spec=curated`, 2020-01-01 to 2026-06-30). All 26
jobs completed with `returncode=0` (~1hr total runtime).

**Result: 0/26 strategies pass `BacktestIntegrityChecker`'s
`check_09_benchmarks` (beat Nifty 500 buy-and-hold)** — this is the
universal blocker, independent of trade count. On top of that:

- **11/26 produced zero trades**: `capital_allocation`,
  `cash_flow_backed_earnings`, `deep_value_solvency`, `longevity`,
  `magic_formula`, `moat`, `net_net`, `owner_earnings`,
  `piotroski_on_value`, `quality_value`, `sector_leader`,
  `turnaround_recovery` — these also fail `check_05_costs`,
  `check_06_liquidity`, `check_08_fold_stability` simply because there's
  no equity curve to evaluate. Root cause confirmed during the queue's
  pre-launch sanity check: 25 of `FUNDAMENTAL_FEATURES` columns
  (`avg_roce_5y`, `delta_roce_3y`, `company_age_years`, and others these
  composites depend on) don't exist in the daily feature panel yet — a
  pre-existing feature-store coverage gap, not a bug introduced by this
  work.
- The remaining 15 strategies generated 1-111 trades but still fail the
  benchmark-beat check; most also fail `check_12_flat_equity_curve`
  (too little trading activity for a meaningful curve) — only
  `small_cap_compounders` (13-111 trades across reruns), `smile` (40
  trades), and `story_numbers` (45 trades) clear that particular check.

**Conclusion: none of the 26 strategies currently qualify to flip
`features/fundamental_composites.py::BACKTESTED_STRATEGIES`** per the
"(a) ran, (b) passed every integrity gate, (c) beat Nifty 500" bar. This
is an honest negative result, not a defect in the adapter/queue
mechanism itself (which is confirmed working end-to-end — trades,
equity curves, and integrity checks all compute correctly for the
strategies with enough feature coverage to trade).

**UPDATE 2026-07-30 — retract the ranking, not just defer it. Root cause:
corrupted raw OHLCV data, not strategy performance.** A `/model-review`
(6 agents, unanimous DO-NOT-SHIP) on these results, prompted by the user
asking why 2024 looked abnormally strong, found that **5 of the 267
total trades across all 26 strategies — 81% of ALL profit generated by
the entire run (₹12.82L of ₹15.84L)** — are a single data-corruption
event: BAJFINANCE and COFORGE (plus SHRIRAMFIN and NAVA, confirmed by
directly scanning `alphalens.duckdb` for the same date pair) show
irregular 2x-10x single-day price jumps between 2024-07-31 and
2024-08-01, with no matching corporate action. Traced to
`ohlcv_ca_audit.raw_close` — the jump is already present in the *raw*
pre-adjustment price, so this is bad source data (NSE bhavcopy for
2024-08-01, or a corruption in receiving/saving it — no local raw
archive survives to pin down which), not a split/bonus-adjustment bug in
`ingestion/adjust/price_adjuster.py`. `backend-data-engineer` confirmed
the per-ticker multipliers are irregular (9.95x, 5.10x, 4.98x, 1.98x),
ruling out a single uniform adjustment-factor bug.

This single date-pair is the entire reason 2024 looks like a blowout
year in the YoY table, and the entire reason `smile` (#1, +48.5% total
return) and `story_numbers` (#2, +38.5%) rank where they do — both
strategies' real, ex-corrupted-trades performance is likely flat or
negative. **Two additional structural gaps compound this:**
- `backtest/integrity_checker.py` has no trade-level sanity check — none
  of its 12 checks look at individual trade PnL/price magnitude, so an
  893%-in-3-days trade passes silently. `backtest-reviewer` recommends
  adding `check_13_extreme_trade_pnl` (flag any trade with `|pnl_pct| >
  ~100%` over a holding period under ~5-10 days).
- `check_09_benchmarks` (the check behind the "0/26 beat Nifty 500"
  headline) may never have received real benchmark data —
  `benchmark_status: "insufficient_benchmark_history"` appears
  **uniformly across all 26/28 runs**, including the 12 zero-trade ones
  that fail it trivially. The "0/26 beat benchmark" finding needs to be
  re-verified as a real result, not assumed.
- `ingestion/scrapers/bhavcopy.py::download_bhavcopy` validates only
  `price <= 0` and `delivery_pct` range at ingestion time — there is no
  day-over-day price-continuity/circuit-breaker check for dates with no
  registered corporate action, so this class of bug will recur silently
  until one is added.

**Action, in order, before any strategy ranking or return figure from
this run is used for a decision:** (1) re-pull 2024-07-31/08-01/08-02
bhavcopy for BAJFINANCE, COFORGE, SHRIRAMFIN, NAVA from a secondary
source (Fyers, per the existing RIGHTS-patch precedent) and repair via
the `ohlcv_ca_audit`/`price_adjuster` restore pattern; (2) add a
generic day-over-day price-continuity sanity gate to
`download_bhavcopy`/`daily_pipeline.py`; (3) add
`check_13_extreme_trade_pnl` to `integrity_checker.py`; (4) confirm
`check_09_benchmarks` is actually being fed a real benchmark series
before citing "0/26 beat benchmark" again; (5) re-run the 26-strategy
queue; (6) flag any strategy trading fewer than ~30 times over the
6.5-year window as statistically inconclusive rather than ranked
(`skeptic-tester`: several strategies have 1-13 trades total). Do not
conclude "fundamental strategies lack edge" from this run — 12/26 never
got a fair test (feature gap, above) and the top 2 are void
...
(no separate helper file was needed — `run_strategy_queue.py`'s own
post-run gate already calls it). Still outstanding from the original
plan (`unified-snacking-zebra.md`) if this is picked back up:
1. Backfill the 25 missing `FUNDAMENTAL_FEATURES` columns (would likely
   unblock the 11 zero-trade strategies).
2. Build the cross-strategy correlation matrix (several strategies are
   near-duplicates by construction — e.g. Moat/Longevity/Sector-Leader
   all lean on `avg_roce_5y`+`margin_stability_5y` — before assuming 26
   "strategies" represent 26 independent signals).
3. Re-run the queue once (1) is addressed and re-check the benchmark-beat
   gate before considering any `BACKTESTED_STRATEGIES` flip.
...
### Remaining actionable REV items closed — 2026-07-21 (follow-up session)

Closed all 9 items left open after the pass above: REV11, REV12, REV15,
REV16, REV17, REV18, REV19, REV26, REV27 — see each row's own updated
note above for the specific fix. F1, F2, FO1-FO4, CA5 remain genuinely
blocked on external data/product decisions and were intentionally not
touched.

Full verification: the entire `tests/unit/` suite (excluding the
resource-heavy gainer/multibagger/deep-model batches, run separately per
`feedback_coverage` convention) — **2221 passed, 4 skipped, 0 failed**,
zero regressions. `tests/quality/` gate battery — **5/5 passed**. Every
new/modified test file also run individually and passing (`test_backtester.py`,
`test_backtest_engine_internals.py`, `test_core_metrics.py`,
`test_core_engine.py`, `test_walk_forward_runner.py`, `test_schema.py`,
...
### A92-A95 — Registry-driven architecture (spec) — 2026-08-13

The canonical schema all four channels converge on. Written down once here so
each channel migration (T15, ML41, ML42, F7) targets the same shape.

**Why this exists.** Strategies, filters and signals have no single source of
truth, so every channel re-declares them: strategy definitions live in four
unrelated shapes, the same seven filter concepts are declared three times, and
no table anywhere records `(strategy, date, ticker, action)`. The consequences
are not cosmetic — a live paper trade cannot be traced back to the signal that
produced it, and nothing structurally prevents a deployed strategy from
diverging from the one that was backtested.

#### `strategy_registry` (A92)

    strategy_key          TEXT     -- canonical {channel}:{name}, matches the UI StrategyKey
    channel               TEXT     -- momentum | technical | fundamental | ml
    name, display_label, description, category
    definition_json       JSON     -- channel-specific setup block
    entry_criterion_json  JSON     -- ordered structured predicates
    exit_criterion_json   JSON     -- policy variant + stop/target/max-hold/trailing
    filter_ids            TEXT[]   -- FKs into filter_registry
    status                TEXT     -- draft | active | retired
    version               INT
    valid_from, valid_to  DATE     -- append-only; never mutate a row in place
    created_at, created_by, source_ref
    PRIMARY KEY (strategy_key, version)

Predicate grammar, shared by every channel, generalizing the condition dicts
`ScreenerTemplate` already uses:

    {"feature": <column>, "op": "lt|gt|lte|gte|eq|between|top_pct|bottom_pct",
     "value": <scalar | [lo, hi]>}

PIT versioning is the load-bearing part: a run records `strategy_key` AND
`version`, so editing a definition cannot silently invalidate historical
results. This is the same discipline already applied to features.

#### `filter_registry` (A93)

    filter_id             TEXT PRIMARY KEY   -- adtv_floor, downtrend, circuit_lock,
                                             -- hmm_regime, quality_gate, chronic_locker, ...
    name, description
    filter_type           TEXT     -- universe | entry | exit | sizing
    params_schema_json    JSON     -- declared params + defaults + valid ranges
    default_params_json   JSON
    applies_to_channels   TEXT[]
    implementation_ref    TEXT     -- dotted path to the ONE implementation
    version, valid_from, valid_to, status

#### `strategy_signals` (A94)

    strategy_key, strategy_version
    signal_date           DATE     -- the as-of date the signal was generated for
    ticker                TEXT
    action                TEXT     -- buy | sell | hold | forced_close
    conviction, rank, size_multiplier
    context_json          JSON     -- sector, adtv_cr, triggering conditions,
                                    -- filter pass/fail
    source                TEXT     -- backtest | live | paper
    run_id                TEXT     -- NULL for live
    generated_at          TIMESTAMP
    PRIMARY KEY (strategy_key, strategy_version, signal_date, ticker, source, run_id)

Persist only EMITTED signals, never every ticker evaluated, and partition by
date — see the row-volume note in A94. All writes go through the existing
`defer_db_writes` path.

#### End state (A95)

- **Backtest** reads definitions and filters from the registries instead of
  importing `TEMPLATES` / `build_category_presets` / `STRATEGY_CATALOG`, and
  writes every signal to `strategy_signals`.
- **API** serves `/api/v1/strategies` and `/api/v1/filters`; the technical
  screener, the alert checker and daily inference read the same rows the
  backtest read.
- **Frontend** stops deriving strategy identity locally —
  `features/backtest-report/strategyKey.ts` and the channel adapters become
  thin reads of `/api/v1/strategies`.
- A strategy's backtested definition and its deployed definition are the same
  row.
...
## Open sub-tasks: ML40 Phase 2 and A95 remainder (noted 2026-08-14)

Both parent rows are 🔧, not ⏳ — the first half of each has landed. These are the
named remainders, in dependency order. They are listed here rather than only inside
the table cells because they are the branch's critical path: A83 cannot close until
ML40-2.1 does, and the A95 quality gate cannot switch on until A95-R1..R3 do.

### ML40 Phase 2 — retire the standalone Momentum engine
- **ML40-2.1** `MomentumBacktester`'s simulation loop (cash, positions, FY tax,
  capital reset) still runs separately from `backtest/core/portfolio.py`. This is
  the blocker for A83: while two simulations exist, one report contract is a
  translation layer over two different computations.
- **ML40-2.2** `momentum_metrics.py` / `momentum_tax.py` still separate from
  `core/metrics.py` / `core/tax.py`. **2026-08-18 investigation — this is NOT the
  delete-and-repoint job the one-line description implied, and it should be done
  BEFORE 2.1:**
  * `core/metrics.py` **already imports `xirr`, `churn_factor` and
    `return_population_zscores` FROM `momentum_metrics.py`**. Those three are
    already the shared cross-channel primitives; only the module's NAME says
    momentum. Retiring the standalone engine without rehoming them would leave
    the shared metrics module importing from a module deprecated for being
    momentum-specific. They belong in `core/metrics.py` (or a neutral module)
    before anything is deleted.
  * The one genuine divergence was **annualization**, now fixed: core's
    `sharpe_ratio` hardcoded 252, correct for the orchestrator's daily equity
    curves and wrong for a per-rebalance momentum curve — a weekly curve read
    ~2.2x too high (sqrt(252/52)). That single mismatch is the whole reason a
    second Sharpe implementation exists. `infer_periods_per_year()` + an optional
    `periods_per_year` argument now let ONE implementation serve every cadence;
    the default is unchanged 252, so no published number moves.
  * **The tax regime was declared twice** — identical `STCG_RATE`/`LTCG_RATE`/
    ...
    retired. Fixed: `core/tax.py` is the single declaration, `momentum_tax.py`
    re-exports.
  * **Still open, and user-visible:** `datastore/api/routers/momentum.py:648`
    computes the Holding Dashboard's "tax due" and "post-tax value" — over REAL
    recorded trades and contributions — through `momentum_tax`'s per-trade,
    winners-only model: no loss set-off, no ₹1.25L LTCG exemption, so
    systematically OVERSTATED, while every orchestrator-driven channel uses
    `core/tax.py`'s FY-netted engine. Same holdings, two tax answers, depending
    on which screen is open. This is the strongest argument for doing 2.2 first:
    it is a live inconsistency, not a refactor.
...
- **ML40-2.5** The three non-adapter knobs, each at the layer it belongs to:
  ...
  momentum, so this needs orchestrator support, not an adapter parameter);
  `investable_pct` is portfolio sizing; `max_pct_of_adtv` already exists generically
  as `StrategyPortfolio.adtv_cap_fraction` (default 0.10) and needs reconciling with
  momentum's per-run value rather than a second cap bolted alongside it. The
...
- **A95-R2** Screener (`systems/technical_analysis/screener/engine.py:53`), the
  ...
  *2026-08-15 progress:* the registry-backed template loader
  (`systems/technical_analysis/screener/registry_templates.py`) is built and
