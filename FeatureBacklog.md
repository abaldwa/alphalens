...
| ML39 | **Production-breaking bug, root-caused to a botched merge in the last commit ever merged to master (36d620d1, 2026-07-23)**: `features/advanced_technical.py::_optimal_fracdiff_d` references `target_adf_threshold` at line 367 but the name is defined nowhere — not a parameter, not a local, not a module constant. Raises a bare `NameError` on every call that reaches a valid ADF computation (any ticker with ≥32 price points and ≥16 valid fracdiff points at some bisection step — essentially all real production data), uncaught anywhere in the call chain (`compute_advanced_technical_features` → `features/matrix_builder.py::build_feature_matrix`). **History** (via `git log -S`/`git show`): branch `fix/remove-unused-adf-threshold-var` (commit c0b82003, 2026-07-13, user-authorized) correctly dropped `target_adf_threshold` as a genuinely-unused parameter — at that time the function used a `scipy.stats.pearsonr` autocorrelation-proxy, not a real ADF test. Separately, commit 971882c6 (2026-07-19, "full-codebase-review Fix 10") rewrote the function on `master` to use a real `statsmodels.tsa.stattools.adfuller` test that genuinely needs the threshold, re-adding `target_adf_threshold: float = -3.5` to the signature. When `fix/remove-unused-adf-threshold-var` was finally merged into master on 2026-07-23 (36d620d1), the merge resolution kept that older branch's signature (parameter removed) but kept master's rewritten body (which still calls `target_adf_threshold` at line 367) — an incomplete/incorrect merge conflict resolution, not a fresh mistake. Confirmed live this session (2026-09-13, scrum-master run): `tests/unit/test_matrix_builder.py` — 9 of 11 tests fail on this exact `NameError` (after also fixing an unrelated missing `statsmodels` pin, see requirements/phase3.txt — `adfuller` needs it, was never pinned anywhere). | ML Signal Engine / Features | ⏳ | The fix is likely mechanical (re-add `target_adf_threshold: float = -3.5` as a parameter/default, matching commit 971882c6's pre-merge version — no caller passes it explicitly today, confirmed via grep, so restoring the old default is very unlikely to be a behavior change from what was actually running before the bad merge) but this is still a `features/` (feature-engineering/ML-core) code change, off-limits for this session's non-intrusive-only charter. Logged here rather than fixed; needs a session with feature/model-core write scope to apply the one-line fix and confirm `test_matrix_builder.py` goes green. |
