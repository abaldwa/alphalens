...
| ML22 | Merge Daily Insights and Daily WatchList screens — significant column/purpose overlap | Dashboard (ML) | ⏳ | **2026-07-13: user decision received** — keep all non-duplicated columns from both screens; duplicate/overlapping columns collapse to one; anything genuinely not needed gets dropped in a later pass once the merged screen is live. Ready to implement. |
...
| ML29 | Sector accumulation detection: (sum of each stock's delivery % × volume) / sector's total outstanding shares, tracked daily, to surface sectors under constant accumulation; drill-down by clicking a sector's %age | Features / Dashboard (ML) | ⏳ | **2026-07-13: user decision received** — sector's total outstanding shares = simple sum of each constituent stock's own outstanding-shares figure (e.g. Stock A 100,000 + Stock B 200,000 = 300,000 for a 2-stock sector), not a weighted or market-cap-based measure. Ready to implement. |
...
| ML33 | Explore survival-curve output (mb_survival_*-style timing distribution, not just a binary hit/miss) for the 21d/63d gainer signal models, mirroring MultiBagger's RandomSurvivalForest approach | ML Signal Engine | 🔧 | 2026-07-13 investigation (experimental `_gainer` copy only, no production impact either way): feasibility confirmed — `training/labeling.py::compute_fixed_pct_labels` would need one addition (a `first_touch_day` field: the day index the +target_pct touch happened, NaN/censored-at-horizon if it never did) to produce the same `(duration, event)` pair survival models need, matching multibagger's `(duration_months, event)` shape. Assessed per-horizon value: low for 6d (window too short for day-level timing to change a trading decision), plausible for 21d/63d. Cost check: unlike multibagger's crippling ~0.3% positive rate over 629K rows, 21d/63d have a much healthier ~26-35% positive rate over a far smaller chunked dataset, so RSF here should be cheap (minutes, not days). Architecturally needs its own small RSF head bolted onto the 21d/63d gainer models plus its own training entry point, run sequentially (not concurrently) with any multibagger retrain to avoid resource contention. **2026-07-13: user authorized development** — build the RSF-head extension now; once development is complete and validated, create a dedicated training job/script and schedule it in the scheduler (separate, explicit follow-up step — not to be wired into the live scheduler without a further explicit go-ahead once the model itself is built and reviewed). |
