...
| A46 | Split `daily_pipeline.py`/`pipeline_scheduler.py` monoliths (1869/2488 lines) into per-concern modules | Scheduler | � | 2026-08-15 session: module boundaries designed and submodule stubs created (`pipeline_run_lock.py`, `pipeline_steps.py`, `pipeline_startup.py`, `run_recording.py`, `scheduler_jobs.py`), facade re-export shim designed. Original restoration from a botched edit — the fully working facade+submodules split requires a second dedicated pass where each submodule is built and verified one at a time without the large-file rename+facade-replace approach. The submodule boundaries documented in this session's BuildLog.md entry are the reference for that pass. |
