...
| FE2 | Frontend build has a >500kB minified `ui` chunk (shared library + Recharts + TradingView bundled together); Vite warns to code-split via dynamic `import()` | Dashboard / Frontend | ✅ 2026-09-13 | **2026-09-13 (scrum-master pass):** fixed both halves the item's own description called out. (1) `frontend/src/app/router.tsx`: every one of the 48 page components (all previously static top-of-file imports, so every one of them and everything they import — `recharts`, `lightweight-charts`, the shared `@/lib/ui` primitives — landed in one bundle downloaded on first paint) is now loaded via `React.lazy(() => import(...))`, one dynamic import per route, each wrapped in a `<Suspense fallback=...>` boundary (a minimal "Loading…" fallback, matching this codebase's existing plain-text loading-state convention rather than adding a spinner component). (2) `frontend/vite.config.ts`: added `build.rollupOptions.output.manualChunks` splitting third-party vendor code into its own chunks by library (`vendor-lightweight-charts`, `vendor-recharts`, `vendor-radix`, `vendor-react-router`, `vendor-tanstack`, `vendor-react`, plus a catch-all `vendor`) instead of one shared `ui` bundle — a page that never touches TradingView-style charts now never downloads `lightweight-charts`. **Before/after** (`npm run build`): before — one `dist/assets/index-*.js` at 1,378.41 kB (390.79 kB gzip) plus Vite's ">500 kB chunk" warning; after — 61 total JS chunks (small per-route/shared-code chunks from 0.15 kB–41.64 kB, plus 8 vendor chunks), largest is `vendor-recharts` at 322.65 kB (86.03 kB gzip) — no chunk over 500 kB, warning gone, `tsc -b && vite build` clean with zero errors/warnings (verified via a full `npm ci && npm run build` in this session, not assumed). `npm run lint` (oxlint) has no new errors — only pre-existing-pattern `react-refresh/only-export-components` *warnings* (already present elsewhere in this codebase, e.g. `button.tsx`/`badge.tsx`; oxlint exit code 0 before and after). No backend/Python code touched. Regression coverage: this project has no frontend test runner (no vitest/jest in `frontend/package.json`) so, per this codebase's own precedent for frontend-only changes (see BuildLog.md, "Frontend `tsc -b` and `npm run build` both clean"), verification is the build itself — confirmed clean, and manually confirmed router.tsx's route table is unchanged (same 49 paths, same components, only the import mechanism changed) via diff review. |
