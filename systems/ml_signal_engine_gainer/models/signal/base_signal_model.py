...
class BaseSignalModel(IClassificationModel):
    ...
    def _fit_base_and_meta(self, X: pd.DataFrame, y: pd.Series, lgbm_params: Optional[Dict[str, Any]] = None) -> None:
        ...
        # wrapper hard-requires 0-based contiguous labels at fit time regardless
        # of any explicit num_class/objective setting (verified: it raises
        # "Invalid classes inferred..." even with objective="multi:softprob",
        # num_class=3 set explicitly), so the old fixed 3-slot CLASS_ORDER
        # offset (-1->0, 0->1, 1->2) crashes whenever a fold is missing one
        # class — every one-sided gainer signal fold (FixedPercentLabeler
        # never emits SELL=-1) hits this deterministically; a production
        # triple-barrier fold with zero sell hits in a strong bull-market
        # calendar year could hit it too. self._present_classes records which
        # CLASS_ORDER values were actually seen (in CLASS_ORDER order) so
        # predict_proba() can expand back to the full, stable
        # sell/hold/buy column contract every downstream caller depends on.
