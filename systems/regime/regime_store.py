...
def save_regime_segments(conn, index_name: str, segments: List[RegimeSegment]) -> None:
    """Upserts segments for index_name — (index_name, start_date) is the
    primary key, so re-running the classifier after new price data arrives
    (which can revise the trailing open segment, or supersede it with a
    newly confirmed one) overwrites cleanly rather than accumulating stale
    rows. Does NOT delete segments no longer produced by a fresh
    classification (e.g. if start dates shift) — callers doing a full
    reclassification should DELETE FROM market_regimes WHERE index_name = ?
    first; see recompute_regime_segments() below for that flow.
    """
    for seg in segments:
        conn.execute(
            """
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?)
            ON CONFLICT (index_name, start_date) DO UPDATE SET
                ...
                computed_at = now()
            """,
            [index_name, seg.regime, seg.start_date, seg.end_date, seg.confirmed_date, METHOD_NAME, seg.move_pct],
        )
    logger.info(f"Saved {len(segments)} regime segments for {index_name}")


def recompute_regime_segments(conn, index_name: str, segments: List[RegimeSegment]) -> None:
    """Full reclassification: replace every stored segment for index_name
    with the freshly computed set. Use this (not save_regime_segments)
    whenever segment START dates may have shifted — e.g. a backfill rerun
    after correcting the underlying index_ohlcv data — since upsert alone
    would leave orphaned rows keyed on start dates the new run no longer
    produces."""
    conn.execute("DELETE FROM market_regimes WHERE index_name = ?", [index_name])
    save_regime_segments(conn, index_name, segments)
