...
Permanently excluded (not "not yet ported" — a decided exclusion):
  ...
  R16 — Target-Volatility Weighting. Retired 2026-09-06 (Category B2):
        TargetVolatilityWeighting.compute_weights() algebraically reduces
        to the SAME formula as R14's InverseVolatilityWeighting unless its
        leverage_cap clip actually engages — never observed to engage
        across the 7 tested configs before retirement, so R16 was never a
        genuinely distinct strategy from R14, independent of the B1
        portfolio bug. r16_target_volatility.py's file is KEPT (historical
        reference / parity-checking against past legacy R16 results,
        same convention as R05) but no longer generated by any
        QueueGenerator or campaign_registry.py entry — see
        test_r16_never_generated in tests/test_queue_generators.py.
