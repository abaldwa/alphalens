...
def list_backlog(status: Optional[str] = None, criticality: Optional[str] = None) -> list[dict[str, Any]]:
    ...
    with get_duckdb_connection(db_path=BACKTEST_DUCKDB_PATH, read_only=True) as db:
        query = """
            SELECT
                b.item_id,
                b.title,
...
def create_backlog_item(
    ...
    with get_duckdb_connection(db_path=BACKTEST_DUCKDB_PATH) as db:
        try:
            db.execute(
                """
...
def update_status(item_id: str, status: str, assigned_to: Optional[str] = None) -> dict[str, Any]:
...
def add_dependency(item_id: str, blocks_on_id: str, reason: Optional[str] = None) -> dict[str, Any]:
